...
abstract class Payments extends Action {
    ...
    protected function addPaymentSerial($int_type = 0, $ary_order = array(),$pay_type = 0,$m_id = 0) {
        ...
        if(!empty($ary_ps) && $ary_ps['ps_money'] == $ary_order['o_all_price']){
            ...
            $data = array();
            $data['m_id'] = !empty($m_id) ? $m_id : $_SESSION['Members']['m_id'];
            $data['pc_code'] = $this->code;
            $data['ps_money'] =$ary_order['o_all_price'];
            $data['ps_type'] = $int_type;
            $data['o_id'] = $ary_order['o_id'];
            $data['ps_status'] = 0;
            $data['pay_type'] = $pay_type;
            $data['ps_create_time'] = date('Y-m-d H:i:s');
            $data['ps_update_time'] = date('Y-m-d H:i:s');
            if(!empty($ary_order['type'])){
                $data['ps_buy_type'] = $ary_order['type'];
                $data['ps_buy_nunmber'] = $ary_order['data'];
            }
//            if(!empty($ary_order['type']) && $ary_order['type']==2){
//                $data['ps_buy_type'] = 2;
//                $data['ps_buy_time'] = $ary_order['data'];
//            }
            $int_ps_id = D('PaymentSerial')->data($data)->add();
            if (false == $int_ps_id) {
                $this->error('生成支付序列号失败');
            } else {
                return $ary_order['o_id'];
            }
        }
    ...
    protected function updataPaymentSerial($int_ps_id, $ps_status, $ps_status_sn, $ps_gateway_sn) {
        ...
        $result = D('PaymentSerial')->where(array('o_id'=>$int_ps_id))->data($data)->save();
        if(false == $result){
            return false;
        }else{
            return true;
