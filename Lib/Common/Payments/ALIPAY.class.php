...
class ALIPAY extends Payments implements IPayments {
    ...
    public function pay($str_oid,$type=2,$data=2,$m_id=null) {
        ...
        require_once "AlipayDirect/lib/alipay_submit.php";
        require_once "AlipayDirect/alipayconf.php";
        $ary_order['type']  = $type;
        $ary_order['data']  = $data;
        ...
        //print_r($alipaySubmit);exit;
        $html_text = $alipaySubmit->buildRequestForm($parame,"get", "确认");
        echo $html_text;
    }
    /**
     * 预存款充值
    ...
    public function respond($data) {
		...
		require_once "AlipayDirect/alipayconf.php";
		require_once "AlipayDirect/lib/alipay_notify.php";
		//计算得出通知验证结果
		$alipayNotify = new AlipayNotify($alipay_config);
		$verify_result = $alipayNotify->verifyNotify();
                  writeLog("verify_result: ".json_encode($verify_result),"order_status.log");
        /* 检查数字签名是否正确 */
        if(!$verify_result){
            return FALSE;
        }
