...
class ThdrustLogin{
     ...
     * @date 2013-1-16
     */
    public function getApiThdRequestUrl($code=''){
        $Communications = new Communications();
        $ary_result['grant_type'] = 'authorization_code';
		$ary_shop_data = D('SysConfig')->getCfgByModule('GY_SHOP',1);
		$tmp_url = 'http://'.$_SERVER['HTTP_HOST'].'/Home/User/ApigetToken/';
		if(!empty($ary_shop_data['GY_SHOP_HOST'])){
			$url = trim($ary_shop_data['GY_SHOP_HOST'],'/').'/Home/User/ApigetToken/';
		}
        $ary_result['redirect_uri'] = urlencode($url);
        $ary_result['code'] = $code;
        switch($_SESSION['str_type']){
            ...
            case 'WX':
                ...
                $return_token = $Communications->httpGetRequest($str_url,$ary_result);
                if ($return_token) {
                    $json = json_decode($return_token, true);
    ...
    protected function checkRetry($method, $arguments = array()) {
        if (!$this->_retry && in_array($this->errCode, array('40014', '40001', '41001', '42001'))) {
            writeLog("ary_result:". json_encode($method),date('Y-md')."weixin_login_Aacceess_token.log");
            $this->errCode = 40001;
    ...
    public function refresh_token($ary_tmp_token){
          $Communications = new Communications();
          $result = $Communications->httpGetRequest('https://api.weixin.qq.com/sns/oauth2/refresh_token?appid='.$this->wx_api_id.'&grant_type=refresh_token&refresh_token='.$ary_tmp_token);
        if ($result) {
    ...
    public function getOauthAccessToken($access_token,$openid){
        $Communications = new Communications();
        //检验授权凭证（access_token）是否有效 https://api.weixin.qq.com/sns/auth?access_token=ACCESS_TOKEN&openid=OPENID
        $res_auth = $Communications->httpGetRequest('https://api.weixin.qq.com/sns/auth?access_token='.$access_token.'&openid='.$openid);
        if ($res_auth) {
            $json = json_decode($res_auth, true);
            if (!$json || !empty($res_auth['errcode'])) {
                $this->errCode = $res_auth['errcode'];
                $this->errMsg = $res_auth['errmsg'];
                return $this->checkRetry(__FUNCTION__, func_get_args());
            }
            return $json;
        }
     ...
     */
    public function getOauthUserinfo($str_thd_user_url, $ary_thd_user) {
        $Communications = new Communications();
        $result = $Communications->httpGetRequest($str_thd_user_url,$ary_thd_user);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getThdWX($str_token){
     ...
     //   $res_auth = $Communications->httpGetRequest('https://api.weixin.qq.com/sns/auth?access_token='.$ary_tmp_token['access_token'].'&openid='.$ary_tmp_token['openid']);
        $ary_auth = $this->getOauthAccessToken($ary_tmp_token['access_token'], $ary_tmp_token['openid']);
       ...
       //     $res_refresh = $Communications->httpGetRequest('https://api.weixin.qq.com/sns/oauth2/refresh_token?appid='.$this->wx_api_id.'&grant_type=refresh_token&refresh_token='.$ary_tmp_token['refresh_token']);
            $ary_tmp_token = $this->refresh_token($ary_tmp_token['refresh_token']);
        }
        ...
        $str_thd_user_url .= '?'.implode('&', $str_param_ary);
        $ary_thd_info =$this->getOauthUserinfo($str_thd_user_url, $ary_thd_user);
