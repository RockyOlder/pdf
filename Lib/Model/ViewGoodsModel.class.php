...
class ViewGoodsModel extends GyfxModel {
    ...
    public function getBrands($tag) {
        ...
        if (isset($tag['num']) && !empty($tag['num'])) {
            $num = $tag['num'];
        } else {
            $num = 100;
        }
    ...
    public function getInfo($cid = null, $is_display = null, $gc_type = 0,$is_cache = false) {
        if($gc_type != 0){
			...
			$data = D('Gyfx')->selectAllCache('goods_category','gc_id as cid,gc_parent_id as fid,gc_name as cname,gc_level as clevel,gc_parent_id,gc_is_display,gc_ad_type,gc_is_hot,gc_pic_url', $where, array('gc_order' => 'asc'));
			//商品类目Url
            foreach ($data as &$c) {
				//楼层
				if($gc_type == '1'){
        ...
        if (null === $this->info) {
			...
			**/
			if($is_cache == true){
				...
				$data = D('Gyfx')->selectAll('goods_category','gc_id as cid,gc_parent_id as fid,gc_name as cname,gc_level as clevel,gc_parent_id,gc_is_display,gc_ad_type,gc_is_hot,gc_pic_url', $where, array('gc_order' => 'asc'));				
			}
			//查询有没有类目促销信息
...
//                    $order_by = 'gi.`g_update_time` desc,gi.g_id desc,g.g_order asc';
    ...
    public function getGoodsList($ary_where, $order_by, $limit, $is_page,$tag,$array_related_spec_detail = array()) {
        ...
        $ary_fields = 'distinct(g.g_id) as gid,gi.`g_description` AS `gdescription`,g.gt_id,gi.`g_market_price` as `maprice`,g.`g_is_prescription_rugs` AS g_is_pres,g_sn as gsn,g_on_sale_time as onsale,g_off_sale_time as offsale
	                             ...
	                             ,g_picture as gpic,g_new as gnew,g_hot as ghot,g_salenum as gsales,gi.`point` AS g_point,gi.`g_picture` as g_picture,gi.g_remark as remark,gi.g_custom_field_1 as field1,gi.g_custom_field_2 as field2,gi.g_custom_field_3 as field3,gi.g_custom_field_4 as field4,gi.g_custom_field_5 as field5';
        //品牌查询
        if (!empty($tag['bid'])) {
    ...
    public function goodDetail($tag,$is_cache = false) {
		...
		$obj_query = $goods
							...
							->field('g.g_id,g.gt_id,g.g_sn,g.g_on_sale_time,g_salenum,g.g_on_sale,
									...
									gi.g_remark,gi.g_desc,g_picture,
