...
class MembersModel extends GyfxModel {
     ...
     * @date 2012-12-11
     */
    public function getInfo($m_name,$open_id=''){
        $ary_return = array();
        if($open_id != ''){
...
//        echo "<pre>";print_r($this->getLastSql());exit;
     ...
     * @date 2012-12-11
     */
    public function checkopenid($open_id){
        $reslut = $this->where(array('open_id'=>$open_id))->find();
        if($reslut){
            return $reslut;
        }else{
            return false;
    ...
    public function getMemberInfoByID($member_id, $fields = '*') {
        ...
        if (empty($member_info)) {
            $member_info = $this->getMemberInfo(array('m_id'=>$member_id),'*',true);
    ...
    {
        $result = $this->table->field($field)->where($condition)->find();
        return $result;
    }
}
