    ...
    public function getCateInfo($tag) {
		...
		foreach ( $data as &$c ) {
			...
			if(!empty($tag['is_show'])){
				...
				$c['list'] = D('Gyfx')->selectAllCache('article','a_id,a_title,a_create_time,hot,a_desc,hits,cat_id,a_is_display',array('cat_id'=>$c ['cat_id'],'a_status'=>1,'a_is_display'=>1,'a_startime'=>array('lt',$date_time),
