...
class GoodsModel extends GyfxModel {
     ...
     */
	 ...
	 public function getDetails($gid=0, $members= array(), $default_pdt_id=0, $qiniu_pic=false) {
         ...
         $ary_goods['gid'] = $gid;
        if (!empty($gid)) {
            $ary_product_feild = array('pdt_sn','pdt_stock','pdt_id', 'pdt_sale_price', 'pdt_market_price', 'g_id', 'pdt_weight','pdt_min_num','pdt_g_remark');
            $where = array();
            $where['g_id'] = $gid;
            $where['pdt_status'] = '1';
            ...
            if (!empty($ary_pdt) && is_array($ary_pdt)) {
                ...
                foreach ($ary_pdt as $kypdt => $valpdt) {
                    ...
                    if($int_num ==1){
                        ...
                        $json_goods_pdts[] = array(
                            ...
                            'pdt_g_remark' => $valpdt['pdt_g_remark'],
                            'pdt_sale_price' => (int)$valpdt['pdt_sale_price'],
                            'pdt_set_sale_price' => (int)$valpdt['pdt_set_sale_price'],
                            'pdt_market_price' => (int)$valpdt['pdt_market_price'],
                            'spd_ids' => '',
                ...
                if(0 == $default_pdt_id) {
                    ...
                    foreach($json_goods_pdts as $goods_pdt) {
                        ...
                        if(empty($goods_pdt['pdt_g_remark'])){
                            $ary_goods['ary_goods_number_pdt'] = $goods_pdt;
                        }
                    }
                }
