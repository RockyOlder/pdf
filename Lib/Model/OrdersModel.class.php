...
//        echo "<pre>";print_r($this->orders_items->getLastSql());exit;
     ...
     * @date 2013-01-07
     */
    public function getOrdersInfoFind($ary_where = array(), $ary_field = '',$group='') {
        $ary_orders_info = M('view_orders',C('DB_PREFIX'),'DB_CUSTOM')->field($ary_field)->where($ary_where)->group($group)->find();
        return $ary_orders_info;
    }
    ...
    public function orderPayment($int_oid, $ary_order = array(), $int_type = 1) {
        ...
        if (isset($int_oid)) {
            ...
            if (false == $int_order_update) {
                writeLog("order save state fail o_id: {$int_oid} ". json_encode($ary_order),date('Ymd')."order_status.log");
...
//			}
            ...
            if (false === $int_orders_log) {
                writeLog("order_log add fail o_id: {$int_oid} ",date('Ymd')."order_status.log");
            ...
            if (false === $int_return_members) {
                writeLog("Members total_price save fail o_id: {$int_oid} m_id:{$ary_member['m_id']}". json_encode($ary_member_data),date('Ymd')."order_status.log");
...
//			}
            ...
            if (false == $int_running_account) {
                writeLog("The current account record failed o_id: {$int_oid} ". json_encode($ary_account_where),date('Ymd')."order_status.log");
...
//                        echo "<pre>";print_r($delivery_company_info);
	...
	*/
     ...
     * @return array
     */
    public function getCheckoutAvailableCoupons($array_params) {
...
//            $ary_point_res['all_price'] = sprintf("%0.2f",$all_price +$ary_logistic_res['logistic_price']);//失败时页面显示金额显示NAN
        ...
        $other_all_price = $ary_res['all_pdt_price'] - $gifts_point_goods_price;

        $other_point_reward = D('PointConfig')->getrRewardPoint($other_all_price);
        $other_point_reward = ceil(($ary_res ['all_price']/$ary_res['o_goods_all_price'])*$other_point_reward);
