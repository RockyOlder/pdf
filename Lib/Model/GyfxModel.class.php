...
class GyfxModel extends Model{
	...
	public function selectOneCache($table,$ary_field=null, $ary_where=null, $ary_order=null,$time=3600)
	{
		...
		if((!ini_get('memcache.allow_failover') && (C('MEMCACHED_OCS') != true)) || (isset($_SESSION['memcache_on']) && $_SESSION['memcache_on'] == 0) ){
                        if(C('REDIS_STAT') == true) {
                            //根据tag获取缓存key
                            $cache_key = json_encode($str_sql);
                            $cache_key = CI_SN.$cache_key.C("HOST_DOWNLOAD");
                            $cache_key = md5($cache_key);
                            if(!empty($ary_return  = cls_redis::get($cache_key))){
                                ...
                                cls_redis::set($cache_key,json_encode($ary_return_data));
		...
		if((!ini_get('memcache.allow_failover') && (C('MEMCACHED_OCS') != true)) || (isset($_SESSION['memcache_on']) && $_SESSION['memcache_on'] == 0) ){
                        if(C('REDIS_STAT') == true) {
                            ...
                            if(!empty($ary_return  = cls_redis::get($cache_key))){
                                ...
                                $ary_return_data = M($table,C('DB_PREFIX'),'DB_CUSTOM')->where($ary_where)->count();
                                cls_redis::set($cache_key,json_encode($ary_return_data));
                                return $ary_return_data;
                            }
                        } else {
		...
		if((!ini_get('memcache.allow_failover') && (C('MEMCACHED_OCS') != true)) || (isset($_SESSION['memcache_on']) && $_SESSION['memcache_on'] == 0) ){
                        if(C('REDIS_STAT') == true) {
                            ...
                            if(!empty($ary_return  = cls_redis::get($cache_key))){
                                ...
                                $ary_return_data = M("",C("DB_PREFIX"),"DB_CUSTOM")->query($str_sql);
                                cls_redis::set($cache_key,json_encode($ary_return_data));
                                return $ary_return_data;
                            }
                        }else {
