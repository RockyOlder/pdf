                ...
                if (!empty($ary_orders_goods) && is_array($ary_orders_goods)) {
                    ...
                    foreach ($ary_orders_goods as $k => $v) {
                        ...
                        if ($v [0] ['type'] == '4') {
                            ...
                            $ary_goods_num = M("goods_info")->where(array('g_id' => $v ['g_id']))->data(array('g_salenum' => array('exp','g_salenum + 1')))->save();
                            if (!$ary_goods_num) {
                                $orders->rollback();
                                $this->error('销量添加失败', array('失败' => U('Ucenter/Orders/OrderFail')));
                                exit();
                ...
                if (!empty($valgoods ['comment'])) {
                    ...
                    if (!$arr_res) {
						...
						if(!empty($point_config['recommend_points'])){
                            ...
                            $ary_reward_result = D('PointConfig')->setMemberRewardPoint($reward_point, $member['m_id'],3);
						...
						if(!empty($point_config['show_recommend_points']) && $isPic === true){
                            ...
                            $ary_reward_result = D('PointConfig')->setMemberRewardPoint($reward_point, $member['m_id'],15);
                            if (!$ary_reward_result ['result']) {
                                M('', C('DB_PREFIX'), 'DB_CUSTOM')->rollback();
                                $this->error($ary_reward_result['message']);exit;
                            }
...
//         echo "<pre>";print_r($data);exit;
    ...
    public function getApplayOid(){
        ...
        if(!empty($data) && $data['o_pay_status'] == 1){
                ...
                $this->ajaxReturn(array('action'=>1,'order'=>$data));
           // $this->ajaxReturn($data);
        } else {
            $this->ajaxReturn(array('action'=>2));
        }
    }
}
