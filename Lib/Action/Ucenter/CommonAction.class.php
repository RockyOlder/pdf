...
abstract class CommonAction extends GyfxAction {
    ...
    public function _initialize() {
        ...
        $arr_get = $this->_get();
        if($arr_get['_URL_']['2'] != 'pageRead' && $arr_get['_URL_']['1'] != 'Notice' && $arr_get['_URL_']['2'] !='doALIPAY' && $arr_get['_URL_']['2'] !='ClientapiALIPAYPayOnline' && $arr_get['_URL_']['2'] !='getOrderPyOid' && $arr_get['_URL_']['2'] !="doAddDepositOnline" && $arr_get['_URL_']['2'] !='synPayReturn'){
            $this->doCheckLogin();
        }
		//获取顶部广告信息
...
//    }
    ...
    private function getMenus() {
        ...
        if($member['admin_id']){
            ...
            $menus[3][30] = array('name' => '收支明细', 'url' => U('Ucenter/Financial/pageDepositList'),'type'=>'1,2,3'); //消费记录
            $menus[3][40] = array('name' => '我的试用申请', 'url' => U('Ucenter/Apply/pageList'),'type'=>'1,2,3'); //试用申请
            $menus[3][50] = array('name' => '我的试用报告', 'url' => U('Ucenter/Report/pageList'),'type'=>'1,2,3'); //试用报告
        ...
        //print_r($ary_menu);exit;
        if(!empty($ary_menu) && is_array($ary_menu)){
            foreach($ary_menu as $keymu=>$valmu){
                if($member['m_type'] != 0 && ($valmu['name'] == "第三方平台" || $valmu['name'] == "推广销售")){
                    continue;
                }
                $arr_menu = D('Gyfx')->selectAllCache('menus',$ary_field=null, array('group'=>'Ucenter','mstatus'=>'1','fid'=>$valmu['id']), $ary_order=null,$ary_group=null,$ary_limit=null);
                ...
                if(!empty($arr_menu)){
                    foreach($arr_menu as $kymu=>$vlmu){
                        ...
                        if(in_array(C('CUSTOMER_TYPE'),$ary_types)){
                            ...
                            $arr_menus = D('Gyfx')->selectAllCache('menus',$ary_field=null, array('group'=>'Ucenter','mstatus'=>'1','fid'=>$vlmu['id']), $ary_order=null,$ary_group=null,$ary_limit=null);
