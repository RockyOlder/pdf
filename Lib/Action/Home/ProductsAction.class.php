...
class ProductsAction extends HomeAction {
    ...
    public function index() {
		...
		if(!empty($ary_request['cid'])){
			$gc_info = D('Gyfx')->selectOneCache('goods_category','gc_name', array('gc_id'=>$ary_request['cid']));
			$gc_name = $gc_info['gc_name'];
		}
		
		if(!empty($ary_request['bid'])){			
			$gb_info = D('Gyfx')->selectOneCache('goods_brand','gb_name', array('gb_id'=>$ary_request['bid']));
...
//        dump($ary_goods_pdts);die;
    ...
    public function getDetailSkus() {
        ...
        $members = session('Members');
		$tpl = C('TMPL_PARSE_STRING.__TPL__'). 'goodsDetailSku.html';
		switch($item_type) {
			case 5:
    ...
    public function ConversionFeeDetail(){
  //      echo urlencode($_SERVER['REDIRECT_URL']);exit;
               if (!session('?Members')) {
			//未登录用户引导到登录页面
			$int_port = "";
			...
			$is_on = D('SysConfig')->getConfigs('GY_SHOP', 'GY_SHOP_OPEN');
			if ($is_on['GY_SHOP_OPEN']['sc_value'] == '0') {
                redirect(U('Home/Index/showMemberInfo?redirectCode=Pay&redirect='.base64_encode($_SERVER['REDIRECT_URL'])));
        ...
        $this->setTitle('充值','TITLE_INDEX','KEY_INDEX','DESC_INDEX');
        $ary_member = D("Members")->where(array('m_id'=>$_SESSION['Members']['m_id']))->find();
        $ary_member['end_time_count_out']  = 0;
        if(time() > strtotime($ary_member['end_time'])){
                $ary_member['end_time_count']  = 0;
                $ary_member['end_time_count_out'] =  count_days(strtotime(date('Y-m-d')),strtotime($ary_member['end_time']));
                    $out_day_small =  time() - strtotime($ary_member['end_time']);
                    if($out_day_small < 86400){
                       $ary_member['end_time_count_out'] = 1;
                    } 
                if($ary_member['end_time_count_out'] > 1000){
                    $ary_member['end_time_count_out']  = 0;
                }
        } else {
                $ary_member['end_time_count']  = count_days(strtotime(date('Y-m-d')),strtotime($ary_member['end_time']));
        }
        $beginThismonth=mktime(0,0,0,date('m'),1,date('Y'));
        $endThismonth=mktime(23,59,59,date('m'),date('t'),date('Y'));
        $PaymentSerialCount  = D('PaymentSerial')->where(array('m_id'=>$_SESSION['Members']['m_id'],'ps_status'=>1,'ps_buy_type'=>1,'ps_buy_nunmber'=>1,'ps_create_time'=>array('between',array(date('Y-m-d H:s:i',$beginThismonth),date('Y-m-d H:s:i',$endThismonth)))))->count();
        $this->assign('PaymentSerialCount', $PaymentSerialCount);
        $info  = D('Goods')->field('g_id')->find();
        $goods_products = M('goods_products',C('DB_PREFIX'),'DB_CUSTOM')->where(array('pdt_min_num'=>1))->find();
      //  print_r($goods_products);exit;
        $members = session('Members');
        $this->assign('ary_member', $ary_member);
        $ary_goods_pdts = D('Goods')->getDetails($info['g_id'], $members,$goods_products['pdt_id']);
        //print_r($ary_goods_pdts);exit;
        $this->assign($ary_goods_pdts);
        $tpl = './Public/Tpl/' . CI_SN . '/' . TPL . '/ConversionFeeDetail.html';
        // echo "<pre>";print_r($ary_goods);exit;
        $this->display($tpl);
    }
    ...
    public function ClientapiConversionFeeDetail() {
        ...
        if( !empty($this->_get('token'))){
            ...
            if(empty($mb_user_token_info) || $mb_user_token_info['ip'] != getIp()){
                  redirect(U('Home/Index/NewWeixinLogin_page'));
            }
            $members = D('Members')->getMemberInfoByID($mb_user_token_info['m_id']);
        ...
        $this->setTitle('充值','TITLE_INDEX','KEY_INDEX','DESC_INDEX');
        $ary_member = D("Members")->where(array('m_id'=>$members['m_id']))->find();
        $ary_member['end_time_count_out']  = 0;
        if(time() > strtotime($ary_member['end_time'])){
                $ary_member['end_time_count']  = 0;
                $ary_member['end_time_count_out'] =  count_days(strtotime(date('Y-m-d')),strtotime($ary_member['end_time']));
      ...
      //  print_r($goods_products);exit;
        ...
        //print_r($ary_goods_pdts);exit;
        $this->assign($ary_goods_pdts);
        $tpl = './Public/Tpl/' . CI_SN . '/' . TPL . '/ClientapiConversionFeeDetail.html';
        // echo "<pre>";print_r($ary_goods);exit;
        $this->display($tpl);
    }
    ...
    public function success(){
        ...
        $this->assign('arr_order',$data);
        $this->assign('token',$this->_get('token'));
        $this->assign('client',$this->_get('client'));
        $tpl = './Public/Tpl/' . CI_SN . '/' . TPL . '/success.html';
        // echo "<pre>";print_r($ary_goods);exit;
        $this->display($tpl);
...
// 客户端取消支付页面
    public function canselpay_details(){
        ...
        if( !empty($this->_get('token'))){
            ...
            $members = D('Members')->getMemberInfoByID($mb_user_token_info['m_id']);
            $this->assign('token', $this->_get('token'));
            $this->assign('o_id', $this->_get('o_id'));
        } else {
            $members = session('Members');
        }
        $tpl ='./Public/Tpl/' . CI_SN . '/' . TPL . '/canselpay_details.html';
        $this->display($tpl);         
    }
    public function send_order_feedback(){
        if( !empty($this->_post('token'))){
            $model_mb_user_token = D('MbUserToken');
            $mb_user_token_info = $model_mb_user_token->getMbUserTokenInfoByToken($this->_post('token'));
            if(empty($mb_user_token_info) || $mb_user_token_info['ip'] != getIp()){
                  redirect(U('Home/Index/NewWeixinLogin_page'));
            }
            $members = D('Members')->getMemberInfoByID($mb_user_token_info['m_id']);
            $this->assign('token', $this->_post('token'));
        } else {
            $members = session('Members');
        }
        $add_member_order_feedback = array();
        $add_member_order_feedback['m_id'] = $members['m_id'];
        $add_member_order_feedback['o_id'] = $this->_post('o_id');
        $add_member_order_feedback['content'] = $this->_post('content');
        $add_member_order_feedback['c_create_time'] = date('Y-m-d H:i;s');
        $add_member_order_feedback['problem'] = $this->_post('state');
        D('OrderFeedback')->add($add_member_order_feedback);
        $redirect_uri = '/Home/Products/ClientapiConversionFeeDetail';
        $this->redirect($redirect_uri,array('token'=>$this->_post('token')));
    }
    
        // 价格服务详情
    public function Price_service_details(){
        $tpl ='./Public/Tpl/' . CI_SN . '/' . TPL . '/Price_service_details.html';
        $ary_member = D("Members")->where(array('m_id'=>$_SESSION['Members']['m_id']))->find();
        $this->assign('ary_member', $ary_member);
        $this->display($tpl);         
    }
