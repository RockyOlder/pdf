...
//              $SelectTokenDataFind =  D('MemberCookie')->where(array('m_id'=>$_SESSION['Members']['m_id']))->find();  
...
//        $this->doCheckLogin();
//        //必须登录跳到登录页
//        if($_SERVER['REQUEST_URI'] !== '/Home/Cart/Pagelist' && $_SERVER['REQUEST_URI'] !='index.php/Home/Cart/Pagelist'){
//            if($shop_close['GY_MUST_LOGIN'] == 1 && !session('?Members') && $_SERVER['REQUEST_URI'] !== '/Wrong' && $_SERVER['REQUEST_URI'] !== '/Home/User/login' &&  $_SERVER['REQUEST_URI'] !== 'index.php/Home/User/login'){
...
//        }
        ...
        if(!empty($this->_get('source'))){
            $_SESSION['str_type'] = $this->_get('source');
            if (!empty($_SESSION['Members']['m_id'])) {
                ...
                if(!D('RelatedMembersSourcePlatform')->where(array('sp_id'=>$source_id,'m_id'=>$_SESSION['Members']['m_id']))->count()){
                    D('RelatedMembersSourcePlatform')->add(array('sp_id'=>$source_id,'m_id'=>$_SESSION['Members']['m_id']));
                }
            }
        }
     //   $this->getOnlineService();
       ...
       // $array_config = D("SysConfig")->where(array("sc_key" => 'GY_TEMPLATE_DEFAULT'))->find();
		...
		$array_config = D('SysConfig')->getCfgByModule('GY_TEMPLATE_DEFAULT',1);
              //  print_r($array_config['GY_TEMPLATE_DEFAULT']);
