...
class OrdersAction extends AdminAction {
    ...
    public function pageList() {
        ...
        if (!empty($ary_get['m_name']) && isset($ary_get['m_name'])) {
            $ary_where[C("DB_PREFIX") . 'members.m_name'] = array('like',"%".$ary_get['m_name']."%");
			$join_where[] = " " . C("DB_PREFIX") . "members ON " . C("DB_PREFIX") . "members.m_id=" . C("DB_PREFIX") . "orders.m_id";
        }
        //如果需要根据充值类型进行搜索
//        if (!empty($ary_get['ps_buy_nunmber']) && isset($ary_get['ps_buy_nunmber'])) {
//            if($ary_get['ps_buy_nunmber'] == 1){
//                 $ary_where[C("DB_PREFIX") . 'payment_serial.ps_buy_type'] = 1;
//            }else {
//                   $ary_where[C("DB_PREFIX") . 'payment_serial.ps_buy_type'] = 2;
//            }
//            if($ary_get['ps_buy_nunmber'] == 2){
//                 $ary_where[C("DB_PREFIX") . 'payment_serial.ps_buy_nunmber'] = 1;
//            } else {
//                $ary_where[C("DB_PREFIX") . 'payment_serial.ps_buy_nunmber'] = $ary_get['ps_buy_nunmber'];
//            }
//            $join_where[] = " " . C("DB_PREFIX") . "payment_serial ON " . C("DB_PREFIX") . "payment_serial.o_id=" . C("DB_PREFIX") . "orders.o_id";
//           // $str_fields .=" ,".C("DB_PREFIX")."payment_serial.ps_buy_nunmber,".C("DB_PREFIX")."payment_serial.ps_buy_type ";
//        }
        //如果需要根据收货人进行搜索
        ...
        //如果需要根据支付入口进行搜索
        if (!empty($ary_get['o_source']) && isset($ary_get['o_source'])) {
            $ary_where[C("DB_PREFIX") . 'orders.o_source'] =  $ary_get['o_source'];
           // print_r($ary_where);exit;
        }
        if (!empty($ary_get['o_source_type']) && isset($ary_get['o_source_type'])) {
            $ary_where[C("DB_PREFIX") . 'orders.o_source_type'] =  $ary_get['o_source_type'];
           // print_r($ary_where);exit;
        }
        //如果需要根据支付状态进行搜索
		...
		//根据订单商品id搜索
        if(!empty($ary_get['pdt_id']) && isset($ary_get['pdt_id'])){
            $ary_where[C("DB_PREFIX").'orders_items.pdt_id'] = $ary_get['pdt_id'];
        }
		if(!empty($ary_get['oi_g_name']) || isset($ary_get['oi_ship_status']) || isset($ary_get['pdt_id'])){
		...
		// 	if(empty($ary_where['fx_orders.o_create_time'])){
		// 		$ary_where[C("DB_PREFIX") . 'orders.o_create_time'] = array("EGT", date("Y-m-d H:i:s",strtotime("-1 month")));
		// 	}
		// }
        //如果要根据客服名称搜索
        ...
        foreach ($ary_orders_info as $k => $v) {
            ...
            ////获取会员邮箱
            $ary_orders_info[$k]['m_email'] = D("Members")->where(array("m_id" => $v["m_id"]))->getField("m_email");
            //获取会员名称
...
//            $payment_serial = M('payment_serial',C('DB_PREFIX'),'DB_CUSTOM')->where(array('o_id'=>$v['o_id']))->field('ps_buy_type,ps_buy_nunmber')->find();
//            $ary_orders_info[$k]['ps_buy_type'] = $payment_serial['ps_buy_type'];
//            $ary_orders_info[$k]['ps_buy_nunmber'] = $payment_serial['ps_buy_nunmber'];
...
//            echo M('orders_items',C('DB_PREFIX'),'DB_CUSTOM')->getLastSql();
            ...
            $pdf_order_id = M('orders_items')->where(array('o_id'=>$v['o_id']))->getField('pdt_id');
            switch ($pdf_order_id){
                case 1:
                    $ary_orders_info[$k]['pdf_type_order']  = '次数';
                    break;
                case 2:
                    $ary_orders_info[$k]['pdf_type_order']  = '一月';
                    break;
                case 3:
                    $ary_orders_info[$k]['pdf_type_order']  = '三月';
                    break;  
                case 4:
                    $ary_orders_info[$k]['pdf_type_order']  = '六月';
                    break;
                case 5:
                    $ary_orders_info[$k]['pdf_type_order']  = '一年';
                    break;
                default :
                    $ary_orders_info[$k]['pdf_type_order']  = '二年';
            }
//			$ary_promotion = array();
...
//        echo "<pre>";print_r($ary_orders_info);exit;
    ...
    public function pageWaitPayOrdersList() {
		...
		if(empty($ary_where['o_id'])){
			$ary_where['o_create_time'] = array("EGT", date("Y-m-d H:i:s",strtotime("-3 month")));
		}
        //数据分页处理，获取符合条件的记录数并分页显示
    ...
    public function pageWaitDeliverOrdersList() {
		$start_time=date("Y-m-d H:i:s",mktime(0,0,0,date("m")-3,date("d"),date("Y")));
        $orders = M('orders', C('DB_PREFIX'), 'DB_CUSTOM');
