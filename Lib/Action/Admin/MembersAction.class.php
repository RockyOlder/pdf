	...
	public function pageList() {
		...
		if ($ary_data['m_name_type'] == '3' && isset($ary_data['m_name']) && !empty($ary_data['m_name'])) {
			$where['m_mobile'] = encrypt(trim($ary_data['m_name']));
                ...
                if(!empty($ary_data['m_id']) && isset($ary_data['m_id'])){
                    $where['m_id'] = $ary_data['m_id'];
                }
        ...
        foreach ($ary_members as &$member) {
            ...
            //首次充值时间
            $member['ps_create_time'] = D('PaymentSerial')->where(array('m_id'=>$member['m_id'],'ps_status'=>1))->order('ps_create_time asc')->getField('ps_create_time');
            //首次消费时间
            $member['member_authorization_time'] = D('MemberAuthorization')->where(array('m_id'=>$member['m_id'], 'type' => array('neq', 0)))->order('creation_time asc')->getField('creation_time');
            //转换文档总数
            $count = D("PdfList")->where(array('m_id' => $member['m_id'], 'cstate' => array('neq', 0)))->count();
            //首次转换时间
            $pdt_time = D("PdfList")->where(array('m_id' => $member['m_id'], 'cstate' => array('neq', 0)))->order('crate_time asc')->getField('crate_time');
            //最近转换时间
            $pdt_time_last = D("PdfList")->where(array('m_id' => $member['m_id'], 'cstate' => array('neq', 0)))->order('crate_time desc')->getField('crate_time');
            //距离现在相差时间
            $count_time =  count_days($pdt_time_last,$pdt_time);
            //按照公式计算周转换数  (平均每周转换文档数=（总转换文档数/距离第一次使用时间）*7)
            $member['member_count_conversion'] =  ($count / $count_time) / 7;
            //已支付订单
            $member['member_order_pay'] = D("Orders")->where(array('m_id'=>$member['m_id'],'o_pay_status'=>1))->count();
            //待支付订单
            $member['member_order_Waiting_pay'] = D("Orders")->where(array('m_id'=>$member['m_id'],'o_pay_status'=>0))->count();
            //退款订单
            $member['member_order_exit_pay'] = D("Orders")->where(array('m_id'=>$member['m_id'],'o_status'=>2))->count();
...
//            $member['cr_name'] = $str_addr;
	...
	public function pageSet(){
		...
		$data = array_merge($verification,$member,$phone,$email,$memberedit,$exitTime);
    ...
    public function PrepaidRecords(){
		$this->getSubNav(6, 0, 10, '充值记录');
		$m_id = $this->_get('mid');
		$page_no = max(1, (int) $this->_get('p', '', 1));
		$page_size = 20;
                $PaymentSerialCount  = D('PaymentSerial')->where(array('m_id'=>$m_id,'ps_status'=>1))->count();
                $obj_page = new Page($PaymentSerialCount, $page_size);
                $page = $obj_page->show();
                $PaymentSerialSelect = D('PaymentSerial')->where(array('m_id'=>$m_id,'ps_status'=>1))->page($page_no, $page_size)->select();
                $this->assign('tabs', 'PrepaidRecords');
                $this->assign('int_nstart', $PaymentSerialCount); //总页数
		$this->assign('page', $page);
		$this->assign('ary_PaymentSerial', $PaymentSerialSelect);
                $this->display();
    }
