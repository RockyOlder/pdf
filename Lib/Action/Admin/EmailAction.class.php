...
class EmailAction extends AdminAction{
    ...
    public function ceshi(){
        $email = new Mail();
                                                   //        $email = new Mail();
               $xx = 'D:/webserver/ceshipdftword.wchtech.com/uploadFile/currentfile/20170623/4/pdf/周报-李时嘉-第15周 _1498147200.pdf';
       $fileName = iconv('utf-8','gbk//IGNORE',$xx);
  //  $ary_option = D('EmailTemplates')->sendEmailFile('574920453@qq.com',$fileName,'word','pdf');
            //   print_r($ary_option);exit;                                
     //   $email->send($ary_option);
         $info = D('SysConfig')->getEmailCfg();
        $ary_option = array(
            'receiveMail' => '574920453@qq.com',
            'subject' => '这是一封测试邮件',
            'message' => '这是一封测试邮件',
            'from' => $info['GY_SMTP_FROM'],
            'fromName' => $info['GY_SMTP_FROM_NAME'],
            'host' => $info['GY_SMTP_HOST'],
            'port' => $info['GY_SMTP_PORT'],
            'file_path'=>$fileName,
            'smtpAuth' => $info['GY_SMTP_AUTH'],
            'username' => $info['GY_SMTP_NAME'],
            'password' => $info['GY_SMTP_PASS'],
            'isHtml' => true
        );
 //       print_r($ary_option);exit;
    ...
    public function pageTemp(){
        ...
        $str_method = explode('::',__METHOD__);
        $this->display($str_method[1]);
    }
