    ...
    public function selectMembers(){
        ...
        if(!empty($ary_data) && is_array($ary_data)){
            foreach($ary_data as $ky=>$vl){
                ...
                $ary_data[$ky]['conversion_types']  = $ary_data[$ky]['conversion_type'];
                if($ary_data[$ky]['conversion_type'] == 0){
                    $ary_data[$ky]['conversion_type'] = '免费用户';
                }else if($ary_data[$ky]['conversion_type'] == 1){
                     $ary_data[$ky]['conversion_type'] = '次数用户';
                }else {
                    $ary_data[$ky]['conversion_type'] = '包月用户';
                }
        }
...
//        echo M('balance_verify_log',C('DB_PREFIX'),'DB_CUSTOM')->getLastSql();exit;
    ...
    public function checkName(){
        ...
        if(!empty($ary_get['o_id']) && isset($ary_get['o_id'])){
            ...
            //echo "<pre>";print_r($ary_data);exit;
            if(!empty($ary_data) && is_array($ary_data)){
                if($ary_data['o_status'] != 1){
                    $this->ajaxReturn("该订单号已作废,请核实...");
                }else {
    ...
    public function doAddBalanceInfo(){
        ...
        if(!empty($ary_post) && is_array($ary_post)){
            ...
            if($ary_post['bt_id'] == 5){
                ...
                $where['m_id'] = $ary_post['m_id'];
                $where['o_id'] = $ary_post['o_id'];
                $res_data = D("Orders")->field("o_status")->where($where)->find();
                if(empty($res_data)){
                      $this->error("该订单不属于用户");
                    }
            }
            ...
            if(FALSE != $ary_result){
                ...
                if(!empty($ary_post['bi_service_verify']) && $ary_post['bi_finance_verify'] == '1'){
                    ...
                    if(!empty($_SESSION['admin_name'])){
                         $params['u_name'] = $_SESSION['admin_name'];
                    }
                    $this->writeBalanceInfoLog($params);
                }
...
//                    echo "<pre>";print_r($ary_post);exit;
                    ...
                    if($ary_post['bi_finance_verify']){
                        ...
                        if($ary_post['bi_type'] !=5){
                            D("Members")->where(array('m_id'=>$ary_post['m_id']))->data(array('m_balance'=>$m_balance))->save();
                        } else {
                            D("Members")->where(array('m_id'=>$ary_post['m_id']))->data(array('conversion_type'=>$ary_post['conversion_type'],'number_remaining'=>$ary_post['number_remaining'],'end_time'=>$ary_post['end_time']))->save();
                            D("Orders")->where(array('m_id'=>$ary_post['m_id'],'o_id'=>$ary_post['o_id']))->data(array('o_status'=>2))->save();
                        }
                    }
                    D($this->_name)->commit();
                    $this->success("操作成功",U("Admin/BalanceInfo/index"),3);
                }else{
                    D($this->_name)->rollback();
                    $this->error("操作失败");
                }
