...
class CouponAction extends AdminAction {
    ...
    public function pageEditAct() {
        ...
        if($data['c_coupon_good_id'] == 'all'){
            ...
            ->field(C('DB_PREFIX').'goods.g_sn,'.C('DB_PREFIX').'goods_info.g_name,'.C('DB_PREFIX').'goods_info.g_stock,'.C('DB_PREFIX').'goods_info.g_price,'.C('DB_PREFIX').'goods_products.g_id,'.C('DB_PREFIX').'goods_products.pdt_collocation_price,'.C('DB_PREFIX').'goods_products.pdt_sn,'.C('DB_PREFIX').'goods_products.pdt_id,'.C('DB_PREFIX').'goods_products.pdt_stock,'.C('DB_PREFIX').'goods_products.pdt_sale_price')
            ->where(array(C('DB_PREFIX').'goods_products.pdt_id'=>$data['c_coupon_good_id'],'pdt_is_combination_goods'=>0,C('DB_PREFIX').'goods_products.pdt_status'=>1))
            ->select();

            $data['goods_data'] = $ary_goods;
        }
        $ary_res = D('GoodsGroup')->getGoodsGroup();
        $search['cates'] =D("ViewGoods")->getCates();
    ...
    public function doPostActivities(){
        ...
        if(!empty($data['c_condition'])){
            $data['c_coupon_good_id'] = 'all';
        }
        if(!empty($data['pro_pdt_id'])){
            $data['c_coupon_good_id'] = $data['pro_pdt_id'];
        }
        //新增或修改数据 +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        $caid = $data['ca_id'];
        if(empty($caid)){
            ...
            if(!empty($data['pro_pdt_id'])){
                $res_pdt= $CouponInfo->where(array('c_coupon_good_id' => $data['pro_pdt_id'],'ca_status'=>0))->getField('ca_id');
                if (!empty($res_pdt)) {
                    $this->error('该商品已在某个活动中');
                }
            }
