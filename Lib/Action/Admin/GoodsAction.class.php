...
//			$array_search_cond["g_name"] = array('like','%' . $_GET["keywords"] . '%');
    ...
    public function getCollocationInfo(){
         $chose = array();
        $chose['g_name']       = $this->_get('g_name', 'htmlspecialchars,trim', '');
        $chose['pdt_sn']       = $this->_get('pdt_sn', 'htmlspecialchars,trim', '');
        $chose['gcid']         = $this->_get('gs_gcid', 'htmlspecialchars,trim', '');
        $chose['search_brand'] = $this->_get('search_brand', 'htmlspecialchars,trim', '');
        ...
        //品牌查询
        if($chose["search_brand"] != 0){
			$where[C('DB_PREFIX').'goods.gb_id'] = $chose["search_brand"];
        }
        //商品编码查询
        if ($chose['pdt_sn']) {
    ...
    public function searchCouponInfo(){
        ...
        ->where(array(C('DB_PREFIX').'goods.g_status'=>'1',C('DB_PREFIX').'goods_products.pdt_status'=>'1','pdt_is_combination_goods'=>'0',C('DB_PREFIX').'goods_products.pdt_sn'=>$str_g_sn))
		...
		if(empty($ary_goods)){
			$this->ajaxReturn(array('status'=>'error','msg'=>'该商品不存在'));
                        ...
                        if($save == 1){
                                ...
                                $exist_where = array();
                                $exist_where['_string'] = 'FIND_IN_SET('.$gid.',c_coupon_good_id) ';

                                $ary_exist =  M('coupon',C('DB_PREFIX'),'DB_CUSTOM')
                                ->where($exist_where)->field('c_name')->find();
                            if($ary_exist['c_name']){
                                        $this->ajaxReturn(array('status'=>'error','msg'=>'商品'.$ary_goods[0]['g_name'].'已在'.$ary_exist['c_name'].'优惠卷里'));
                            }else{
				//商品详情
