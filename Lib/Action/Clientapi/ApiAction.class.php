...
//             $array_params['token'] = str_ireplace(' ', '+', $array_params['token']);
//
//             $array_params_data = rsa_private_decrypt($array_params['token']);
//             $array_params   = $this->convertUrlQuery($array_params_data);
             $this->array_params  = $array_params;
           // $aa =  rsa_public_encrypt(json_encode($array_params));
          //  echo $aa;exit;
//        writeLog('Erp request: '.var_export($array_params, true), 'erpapi.log');
		...
		if(!isset($array_params["app_key"]) || "" == $array_params["app_key"]){
			$this->errorResult(false,10001,array(),'缺少系统级参数app_key','',true);
		}
		...
		if(SAAS_ON == TRUE){
			...
			try {
				...
				//连接管易数据中心失败
				$this->errorResult(false,10001,array(),'验证管易分销授权中心数据库失败。','',true);
			}
			...
			if($str_app_key != CI_SN){
				$this->errorResult(false,10001,array(),'错误的app_key','',true);
			}
		...
		if(!isset($array_params["timestamp"]) || "" == $array_params["timestamp"]){
			$this->errorResult(false,10001,array(),'缺少系统级参数timestamp','',true);
		}

		//验证是否传递sign参数
		if(!isset($array_params["sign"]) || "" == $array_params["sign"]){
			$this->errorResult(false,10001,array(),'缺少系统级参数sign','',true);
		}
		...
		if($sign != $array_params['sign']){
			$this->errorResult(false,10001,array(),'数据签名不正确','',true);
                ...
                //    $now_date = time();
                //    $data = $array_params["timestamp"];//strtotime();
...
//                    }
	 ...
	 * @param array $options
	 */
		protected function result($status=false,$error_code="",$ary_data = array(),$str_msg="",$options=array(),$key=false){
			if($this->format == 'json'){
				$response = array();
				if($options['root_tag']){
					...
					$response = $ary_data;
                                ...
                                if($key == true){
                                     echo rsa_private_encrypt(json_encode($response));
                                } else {
		 ...
		 *
		 */
		protected function errorResult($status=false,$error_code="",$ary_data = array(),$str_msg="",$msg='Remote service error',$key=false){
			$array_data = array(
			'code'=>$error_code,
			'msg'=>$msg,
			'sub_code'=>$ary_data,
