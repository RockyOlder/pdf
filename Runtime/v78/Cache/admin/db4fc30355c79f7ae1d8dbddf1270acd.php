...
<a href="<?php echo ($bread0["url"]); ?>"><?php echo ($bread0["name"]); ?></a>
 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
    ...
    <form method="get" action="<?php echo U('Admin/GoodsGroup/doDel');?>" id="members_del">
    <table width="100%" class="tbList">
        <thead>
            <tr class="title">
                <th colspan="99">
                     <p class="conOneP" style="float: left;">
                        <a href='<?php echo U("Admin/GoodsGroup/addGroup");?>' class="btnG ico_add">新增商品分组</a>
                    </p>
        ...
        <tbody>
            <?php if(is_array($datalist)): $i = 0; $__LIST__ = $datalist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$group): $mod = ($i % 2 );++$i;?><tr>
                <td><input type="checkbox" class="checkSon" name="ggid[]" value="<?php echo ($group["gg_id"]); ?>" /></td>
                <td>
					<a href="<?php echo U('Admin/GoodsGroup/pageEdit');?>/ggid/<?php echo ($group["gg_id"]); ?>">编辑</a>
					<a href="<?php echo U('Admin/GoodsGroup/doDel');?>?ggid[]=<?php echo ($group["gg_id"]); ?>" class="confirm">删除</a>
				</td>
				<td>
					<input type="text" name="change_order" class="change_order tiny" gg_id="<?php echo ($group["gg_id"]); ?>" order="<?php echo ($group["gg_order"]); ?>" value="<?php echo ($group["gg_order"]); ?>" />
				</td>
				<td><a href='<?php echo U("Admin/Products/pageList","ggid=$group[gg_id]");?>'><?php echo ($group_goods_nums[$group['gg_id']]); ?></td>
                <td style="text-align:left;"><?php echo ($group["gg_name"]); ?></td>
                <td style="text-align:left;"><?php echo ($group["gg_desc"]); ?></td>
            </tr><?php endforeach; endif; else: echo "" ;endif; ?>
            <?php if(empty($datalist)): ?><tr><td colspan="99" class="left">暂时没有数据!</td></tr><?php endif; ?>
        </tbody>
        <tfoot>
            <tr>
...
$(document).ready(function(){
	$(".change_order").change(function(){
		if(!isNaN($(this).val())){
			//POST请求到服务器，将排序修改掉
			$.post('<?php echo U("Admin/GoodsGroup/changeOrder");?>',{gg_id:$(this).attr("gg_id"),gg_order:$(this).val()},function(dataObj){
				//alert("商品分组排序修改成功。");
				if(dataObj.status == true){
					window.location.href = "<?php echo U('Admin/GoodsGroup/pageList');?>/p/<?php echo (isset($_GET['p']) && is_numeric($_GET['p']))?$_GET['p']:1 ?>/t/" + Math.random()*10000;
				}else{
					alert(dataObj.msg);
				}
			},'json');
