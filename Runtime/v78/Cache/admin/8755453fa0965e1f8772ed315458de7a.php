		...
		<thead>
			<tr class="title">
				<th colspan="99">
					...
					名称：<input type="text" class="width_100 search_cond" name="l_name" value="<?php echo ($filter["l_name"]); ?>" />
					
					有效期：<input type="text" class="width_100 timer search_cond" name="l_start_time" value="<?php echo ($filter["l_start_time"]); ?>" />
					- 
					<input type="text" class="width_100 timer search_cond" name="l_end_time" value="<?php echo ($filter["l_end_time"]); ?>" />
					<select name="l_status" id="l_status">
						<option value="-1">是否启用</option>
						<option value="2" <?php if($filter["l_status"] == 2): ?>selected<?php endif; ?>>停用</option>
						<option value="1" <?php if($filter["l_status"] == 1): ?>selected<?php endif; ?>>启用</option>
					</select>
		...
		<form id="coupon_del" method="get" action="<?php echo U('Admin/Lottery/doDel');?>">
    		<tbody>
        		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cp): $mod = ($i % 2 );++$i;?><tr>
        				<td>
                            <input type="checkbox" class="checkSon" name="l_id[]" value="<?php echo ($cp["l_id"]); ?>" />
                        </td>
        				<td>
							<a href='<?php echo U("Admin/Lottery/addLoterys?l_id=$cp[l_id]");?>'>编辑规则</a>
        					<a href='<?php echo U("Admin/Lottery/pageEdit?l_id=$cp[l_id]");?>' class="edit"></a> | 
                            <a href='<?php echo U("Admin/Lottery/doDel?l_id=$cp[l_id]");?>' class="confirm delete"></a>
                        </td>
        				<td><?php echo ($cp["l_name"]); ?></td>
        				<td><?php echo ($cp["l_start_time"]); ?> - <?php echo ($cp["l_end_time"]); ?></td>
        				<td><?php echo ($cp["l_number"]); ?></td>
        				<td><?php echo ($cp["consume_point"]); ?></td>
						<td><?php echo ($cp["l_create_time"]); ?></td>
						<td><?php echo ($cp["l_update_time"]); ?></td>
						<td>
						<?php if($cp["l_status"] == '1'): ?>启用
						<?php else: ?>
						停用<?php endif; ?>
						</td>
    		...
    		<tfoot>
    			<tr>
    				<td colspan="99">
						...
						<a href="<?php echo U('Admin/Lottery/pageAdd');?>" class="btnA" >新增抽奖活动</a>
...
$(document).ready(function(){
	$("#searchButton").click(function(){
		var redirect_url = "<?php echo U('Admin/Lottery/index');?>" + "?";
