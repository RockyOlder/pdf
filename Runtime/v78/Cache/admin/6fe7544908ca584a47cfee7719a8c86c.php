 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
    ...
    <table width="100%" class="tbList">
        ...
        <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>    
                <td><?php echo ($list["bt_name"]); ?></td>
                <td><input type="checkbox" id="list_<?php echo ($bt_code); ?>" bt_code="<?php echo ($list["bt_code"]); ?>" class="bt_status" value="<?php echo ($list["bt_status"]); ?>" <?php if($list["bt_status"] == '1'): ?>checked="checked"<?php endif; ?>/></td>
                <td>
                    <?php echo ($list["bt_desc"]); ?>
                </td>
...
<script>
    $(document).ready(function(){
        ...
        $('.bt_status').click(function(){
            ...
            var url = "<?php echo U('Admin/BalanceType/doStatusBalanceType');?>";
            $.ajax({
                url:url,
                cache:false,
                dataType:'json',
