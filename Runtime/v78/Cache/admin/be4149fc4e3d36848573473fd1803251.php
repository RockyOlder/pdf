...
<a href="<?php echo ($bread0["url"]); ?>"><?php echo ($bread0["name"]); ?></a>
 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
    ...
    <table width="100%" class="tbList" data-uri='<?php echo U("Admin/Distirbution/doStatus");?>'>
        ...
        <tbody>
            <?php if(is_array($ary_shops)): $i = 0; $__LIST__ = $ary_shops;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$shop): $mod = ($i % 2 );++$i;?><tr id="list_<?php echo ($shop["ts_id"]); ?>">
                <td><input type="checkbox" class="checkSon" name="ts_id[]"  value="<?php echo ($shop["ts_id"]); ?>" /></td>
                <td><a href='<?php echo U("Admin/Distirbution/topOauth");?>' >授权更新</a>&nbsp;<a href='<?php echo U("Admin/Distirbution/doDelShops?ts_ids=$shop[ts_id]");?>' class="confirm">删除</a></td>
                <td><?php echo ($shop["ts_title"]); ?></td>
                <td><?php echo ($shop["ts_created"]); ?></td>
                <td><?php echo ($shop["ts_modified"]); ?></td>
                <td><input type="radio" class="ts_default" id="ts_default_<?php echo ($shop["ts_id"]); ?>" name="ts_default" data-uri='<?php echo U("Admin/Distirbution/doEditDistirbutionDefault");?>' data-id="<?php echo ($shop["ts_id"]); ?>" data-field="ts_default" data-value="<?php echo ($shop["ts_default"]); ?>" <?php if(($shop["ts_default"]) == "0"): ?>value="0"<?php else: ?>value="1"  checked="checked"<?php endif; ?> /></td>
        ...
        <tfoot>
            <tr>
                <td colspan="99">
                 <a href='<?php echo U("Admin/Distirbution/topOauth");?>' class="btnA">店铺绑定</a>
                <input type="button" data-uri='<?php echo U("Admin/Distirbution/doDelShops");?>'  value="取消店铺授权" class="btnA confirm" id="delAll" /><span class="right page"><?php echo ($page); ?></span></td>
            </tr>
        </tfoot>
    </table>
    <!--</form>-->
