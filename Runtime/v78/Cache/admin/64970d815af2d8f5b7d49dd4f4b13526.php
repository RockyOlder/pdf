...
<a href="<?php echo ($bread0["url"]); ?>"><?php echo ($bread0["name"]); ?></a>
 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
  ...
  <p class="tabListP">
        <a href='<?php echo U("Admin/Orders/pageDetails?o_id=$int_oid");?>'><span onclick="setTab('tabListP',1,3)" id="tabListP1">订单详情</span></a>
        <a href='<?php echo U("Admin/Orders/pageOrdersLog?o_id=$int_oid");?>'><span onclick="setTab('tabListP',2,3)" id="tabListP2" >订单日志</span></a>
        <a href='<?php echo U("Admin/Orders/pageOrdersReceipt?o_id=$int_oid");?>'><span onclick="setTab('tabListP',3,3)" id="tabListP3" class="onHover">售后单据</span></a>
    </p>
    

    <div class="rightInner" id="con_tabListP_3" ><!--rightInner  start-->
        <table width="100%" class="tbList" style="border-top:1px solid #d7d7d7; margin-top:10px;">
            ...
            <tbody>
            <?php if(is_array($ary_refunds)): $i = 0; $__LIST__ = $ary_refunds;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$refunds): $mod = ($i % 2 );++$i;?><tr>
                    <td><?php echo ($refunds["or_return_sn"]); ?></td>
                    <td><?php echo ($refunds["oi_g_name"]); ?></td>
                    <td><?php echo ($refunds["g_sn"]); ?></td>
                    <td><?php echo ($refunds["pdt_spec"]); ?></td>
                    <td><?php echo ($refunds["ori_num"]); ?></td>
                    <td><?php echo ($refunds['oi_refund_status'] == '4'?'退款':'退货'); ?></td>
                    <td><?php echo ($refunds["or_create_time"]); ?></td>
                    <td><?php echo ($refunds["or_money"]); ?></td>
                    <td><?php echo ($refunds["or_return_logic_sn"]); ?></td>
                    <td><?php echo ($refunds["or_buyer_memo"]); ?></td>
                    <td><?php echo ($refunds["or_seller_memo"]); ?></td>
                    <td><?php echo ($refunds["refund_status"]); echo ($refunds["refund_goods_status"]); ?></td>
