...
<div class="rightInner">
    <form id="coupon_add" enctype="multipart/form-data" name="coupon_add" method="post" action="<?php echo U('Admin/Groupbuy/doAdd');?>" onsubmit="return save();">
		...
		dataType:'json'
                ...
                <tr>
                    ...
                    <select name="gcid" class="small search_cond" style="width: auto">
                             ...
                             <?php if(is_array($cates)): $i = 0; $__LIST__ = $cates;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cate): $mod = ($i % 2 );++$i;?><option value="<?php echo ($cate['gc_id']); ?>" <?php if(($gcid == $cate['gc_id'])): ?>selected=selected<?php endif; ?> ><?php echo ($cate['gc_name']); ?></option>
								<?php if(!empty($cate["sub"])): if(is_array($cate["sub"])): $i = 0; $__LIST__ = $cate["sub"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sub_cate): $mod = ($i % 2 );++$i;?><option value="<?php echo ($sub_cate['gc_id']); ?>" <?php if(($gcid == $sub_cate['gc_id'])): ?>selected=selected<?php endif; ?> >----<?php echo ($sub_cate['gc_name']); ?></option><?php endforeach; endif; else: echo "" ;endif; endif; endforeach; endif; else: echo "" ;endif; ?>
                    </select>	
                    </td>
                    <td class="last">（移动端暂不支持）</td>
                </tr>	
                <tr>
                    ...
                    <select name="gbbid" class="small search_cond" style="width: auto">
                             ...
                             <?php if(is_array($brands)): $i = 0; $__LIST__ = $brands;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$brand): $mod = ($i % 2 );++$i;?><option value="<?php echo ($brand['gbb_id']); ?>" <?php if(($brand == $cate['gbb_id'])): ?>selected=selected<?php endif; ?> ><?php echo ($brand['gbb_name']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                    </select>	
                    </td>
                    <td class="last">（移动端暂不支持）</td>
                </tr>				
                    ...
                    <td colspan="2">
                        <table  id="J_ManagePrice"  class="clearfix dialog manage-cats" style="width: 100%">
                            ...
                            <tr class="price_discount_init" data-type="1">
                                <td>
                                    初始团购价：减<input type="text" name="gp_price"  value="<?php echo (($info['gp_price'])?($info['gp_price']):0); ?>" class="medium input-text" onblur="checkGroupbuyPrice(this);" />元&nbsp;&nbsp;
                            ...
                            <tr class="price_discount_init" data-type="2" style="display: none;">
                                <td>
                                    初始团购价：<input type="text" value="<?php echo ($info['gp_price']); ?>" class="medium input-text" onblur="checkGroupbuyPrice(this);" />折&nbsp;&nbsp;
                                    <img src="__PUBLIC__/Admin/images/u48_normal.png" onclick="addTieredPrice();" title="添加价格阶梯" />
                                </td>
	            ...
	            <tr>
                    ...
                    <td>
                        <input type="checkbox" name="gp_is_baoyou" <?php if($info["gp_is_baoyou"] == '1'): ?>checked<?php endif; ?> id="gp_is_baoyou" value="1" />
                    </td>
                    <td class="last"></td>
                </tr>					
	            <tr>
	            ...
	            <tr>
                    ...
                    <td>
                        <input type="checkbox" name="gp_start_code" <?php if($info["gp_start_code"] == '1'): ?>checked<?php endif; ?> id="gp_start_code" value="1" />
                    </td>
