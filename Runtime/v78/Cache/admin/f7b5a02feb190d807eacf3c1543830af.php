...
<a href="<?php echo ($bread0["url"]); ?>"><?php echo ($bread0["name"]); ?></a>
 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
    ...
    <span id="tabListP1" <?php if($tabs == 'pageEdit'): ?>class="onHover"<?php endif; ?>><a href='<?php echo U("Admin/Members/pageEdit","mid=$m_id");?>'>会员编辑</a></span>
    <span id="tabListP2" <?php if($tabs == 'pointList'): ?>class="onHover"<?php endif; ?> ><a href='<?php echo U("Admin/Members/pointList","mid=$m_id");?>'>积分历史</a></span>
</p>
<div id="content">
   <div class="rightInner" id="tabList1">
       <table class="tbList" width="100%">
            ...
            <tbody>
                            <?php if(is_array($ary_pointlog)): $i = 0; $__LIST__ = $ary_pointlog;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$pointlog): $mod = ($i % 2 );++$i;?><tr>
                                    <td align="center"><?php echo ($pointlog["u_create"]); ?></td>
                                    <td align="center"><b class="yellow"><?php if(!empty($pointlog["reward_point"])): echo ($pointlog["reward_point"]); endif; ?></b></td>
                                    <td align="center"><?php if(!empty($pointlog["consume_point"])): echo ($pointlog["consume_point"]); endif; ?></td>
                                    <td align="center"><?php echo ($pointlog["memo"]); ?></td>
									<td align="center">
									<?php if($pointlog["type"] == 0): ?>购物赠送<?php endif; ?>
									<?php if($pointlog["type"] == 1): ?>购物消费积分<?php endif; ?>
									<?php if($pointlog["type"] == 2): ?>注册奖励积分<?php endif; ?>
									<?php if($pointlog["type"] == 3): ?>评论送积分<?php endif; ?>
									<?php if($pointlog["type"] == 4): ?>订单退货成功还原冻结积分<?php endif; ?>
									<?php if($pointlog["type"] == 5): ?>管理员积分调整<?php endif; ?>
									<?php if($pointlog["type"] == 6): ?>积分冻结<?php endif; ?>
									<?php if($pointlog["type"] == 7): ?>作废订单成功还原冻结积分<?php endif; ?>
									<?php if($pointlog["type"] == 8): ?>订单作废还原冻结积分<?php endif; ?>
									<?php if($pointlog["type"] == 9): ?>抽奖消费积分<?php endif; ?>
									<?php if($pointlog["type"] == 10): ?>每日签到<?php endif; ?>
									<?php if($pointlog["type"] == 11): ?>金币兑换积分<?php endif; ?>
									<?php if($pointlog["type"] == 12): ?>积分兑换金币<?php endif; ?>
									<?php if($pointlog["type"] == 13): ?>登陆获赠积分<?php endif; ?>
									<?php if($pointlog["type"] == 14): ?>推荐注册送积分<?php endif; ?>
									<?php if($pointlog["type"] == 15): ?>晒单送积分<?php endif; ?>
                                    <?php if(is_array($members_fields)): $i = 0; $__LIST__ = $members_fields;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$fields): $mod = ($i % 2 );++$i; $typeId = $fields['id']+100; ?>
                                    <?php if($pointlog["type"] == $typeId): ?>完善会员信息<?php echo ($fields["field_name"]); ?>送积分<?php endif; endforeach; endif; else: echo "" ;endif; ?>
									</td>
