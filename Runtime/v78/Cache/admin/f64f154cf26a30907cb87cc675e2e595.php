...
<a href="<?php echo ($bread0["url"]); ?>"><?php echo ($bread0["name"]); ?></a>
 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
	...
	<table width="100%" class="tbList">
				...
				<th colspan="99" >
                    <ul class="conOneUl">
                        <form action="/Admin/Coupon/pageActList" id="card_search" method="get">
                            <li>名称：<input type="text" class="width_100 search_cond" name="ca_name" value="<?php echo ($filter["ca_name"]); ?>" /></li>
		...
		<form id="coupon_del" method="get" action="<?php echo U('Admin/Coupon/delCouponAct');?>">
    		<tbody>
        		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cp): $mod = ($i % 2 );++$i;?><tr>
        				<td>
                            <input type="checkbox" class="checkSon" name="ca_id[]" value="<?php echo ($cp["ca_id"]); ?>" />
                        </td>
        				<td>
        					<a href='<?php echo U("Admin/Coupon/pageEditAct?ca_id=$cp[ca_id]");?>' class="edit"></a> | 
                            <a href='<?php echo U("Admin/Coupon/delCouponAct?ca_id=$cp[ca_id]");?>' class="confirm delete"></a>
                        </td>
        				<td><?php echo ($cp["ca_name"]); ?></td>
        				<td><?php echo ($cp["ca_start_time"]); ?> - <?php echo ($cp["ca_end_time"]); ?></td>
        				<td><?php echo ($cp["ca_total"]); ?></td>
        				<td><?php echo ($cp["ca_used_num"]); ?></td>
        				<td>
        					<?php if($cp["ca_type"] == 0 ): ?>同号券
                            <?php elseif($cp["ca_type"] == 1): ?>
                            异号券
                            <?php else: ?>
                            注册券<?php endif; ?>
                        </td>
        				<td><?php echo ($cp["ca_memo"]); ?></td>
        				<td>
        					<?php if(($cp["ca_status"]) == "1"): ?><span class="red">未启用</span>
                            <?php else: ?>
                                <span class="green">已启用</span><?php endif; ?>
        				</td>
...
$(document).ready(function(){
	$("#searchButton").click(function(){
		var redirect_url = "<?php echo U('Admin/Coupon/pageActList');?>" + "?";
