 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
	...
	<table width="100%" class="tbList">
		<thead>
			<tr class="title">
				<th colspan="99">
				<form method="post" action="<?php echo U('Admin/Try/report');?>">
					...
					<span style="float: right;">
                        ...
                        <select name="reportStatus" class="small search_cond" style="width:auto" >
                            <option value="0" <?php if(($filter["reportStatus"] == 0)): ?>selected=selected<?php endif; ?> >未审核</option>
                            <option value="1" <?php if(($filter["reportStatus"] == 1)): ?>selected=selected<?php endif; ?> >已审核</option>
                        </select>
		...
		<tbody>
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ay): $mod = ($i % 2 );++$i;?><tr>
                    <td>
                        <input type="checkbox" class="checkSon" name="tr_id" value="<?php echo ($ay["tr_id"]); ?>" />
                    </td>
                    <td>
                        <span style="display:block;width:80px;">
                            <?php if($ay['tr_status'] == 0): ?><a href="javascript:void(0);" tr_id="<?php echo ($ay["tr_id"]); ?>" data-uri='<?php echo U("Admin/Try/ReportAudit");?>' class="Audit">审核</a>
                                <!-- <a href='javascript:void(0);' tar_id="<?php echo ($ay["tar_id"]); ?>" data-uri='<?php echo U("Admin/Try/ShowApplyQuestion");?>' class="setTry">查看问题</a>&nbsp;&nbsp; -->
                            <?php else: ?>
                                <span style="color:blue;">已审核</span><?php endif; ?>
                            <a href='javascript:void(0);' tr_id="<?php echo ($ay["tr_id"]); ?>" data-uri='<?php echo U("Admin/Try/ReportDetail");?>' class="showDetail">查看详情</a>
                            <div id="children_<?php echo ($ay["tr_id"]); ?>"  style="display:none" title="详情"></div>
                        </span>
                    </td>
                    <td><?php echo ($ay["try_title"]); ?></td>
                    <td><?php echo ($ay["m_name"]); ?></td>
                    <td><?php echo ($ay["tr_create_time"]); ?></td>
					<td>
...
$(document).ready(function(){
	$("#searchButton").click(function(){
		var redirect_url = "<?php echo U('Admin/Try/report');?>" + "?";
