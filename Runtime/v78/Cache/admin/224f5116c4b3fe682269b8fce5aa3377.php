...
<div class="rightInner">
    <table width="100%" class="tbList">
        <thead>
            <tr class="title">
                ...
                <th colspan="99" style="text-align:right;font-size: 12px;">
                    <form id="searchForm" method="get" action="<?php echo U('Admin/Orders/pageWaitPayOrdersList');?>">
        ...
        <tbody>
        <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$order): $mod = ($i % 2 );++$i;?><tr id="oid_<?php echo ($order["o_id"]); ?>">
                ...
                <td>
                    ...
                    <?php else: ?>         	
                        ...
                        <?php else: ?>
							...
							<?php if($order["verify_pay_status"] > 0): ?><a href='javascript:void(0);' style="color:gray;" title="已提交支付申请,等待审核">已提交支付申请,等待审核</a>
							<?php else: ?>
							<a href='javascript:void(0);' onClick="payOrder(this);" data-uri='<?php echo U("Admin/Orders/payOrder");?>' oid="<?php echo ($order["o_id"]); ?>" <?php if($order["deliver_status"] == '已发货'): ?>title="此订单已发货" style="color:red;"<?php endif; ?>>支付</a><?php endif; ?>
							</span>
							 <div id="children_<?php echo ($order["o_id"]); ?>"  style="display:none" title="支付申请"></div><?php endif; ?>
                                
                        <!-- 订单的发货状态 start -->
                        <?php if($order["deliver_status"] == '已发货'): ?><span style="margin-left:10px;">已发货</span>
                        <?php else: ?>
                            <span style="margin-left:10px;">未发货</span><?php endif; endif; ?>
				</td>
