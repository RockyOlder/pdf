...
<a href="<?php echo ($bread0["url"]); ?>"><?php echo ($bread0["name"]); ?></a>
 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
    ...
    <form action="<?php echo U('Admin/GoodsProperty/doDelSpec');?>" method="POST" onsubmit="return beforeDeleteConfirm();">
    <table width="100%" class="tbList">
                ...
                <th colspan="99">
					...
					<span style="margin-left:30px;">
						<select name="gt_id" class="medium" id="goods-spec-list-type-change">
							...
							<?php if(is_array($array_type_info)): $i = 0; $__LIST__ = $array_type_info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$type): $mod = ($i % 2 );++$i;?><option value="<?php echo ($type['gt_id']); ?>" <?php if($int_type_id == $type['gt_id']): ?>selected<?php endif; ?>><?php echo ($type['gt_name']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
						</select>
					</span>
                </th>
            </tr>
        ...
        <tbody>
            <?php if(is_array($ary_spec)): $i = 0; $__LIST__ = $ary_spec;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$spec): $mod = ($i % 2 );++$i;?><tr>
                <td><input type="checkbox" class="checkSon" name="gs_id[]" <?php if($spec["gs_is_system_spec"] == 1): ?>disabled="disabled"<?php endif; ?> value="<?php echo ($spec["gs_id"]); ?>" /></td>
                <td>
					<?php if($spec["gs_is_system_spec"] != 1): ?><a href='<?php echo U("Admin/GoodsProperty/specEditPage?gsid=$spec[gs_id]&gt_type=$spec[gt_type]");?>'>编辑</a> 
                    <a href='<?php echo U("Admin/GoodsProperty/doDelSpec?gsid=$spec[gs_id]&gt_type=$spec[gt_type]");?>' class="confirm">删除</a>
					<a href='<?php echo U("Admin/GoodsProperty/addSpecPage?gt_id=$spec[gt_id]&gt_type=$spec[gt_type]");?>'>添加同类属性</a>
					<?php else: ?>
					<a href="javascript:void(0);" style="color:gray;" title="系统属性，不允许编辑。">编辑</a>
					<a href="javascript:void(0);" style="color:gray;" title="系统属性，不允许删除。">删除</a><?php endif; ?>
                </td>
                <td><?php echo ($spec["gs_name"]); ?></td>
                <td><?php if(($spec["gs_is_system_spec"]) == "0"): ?>否 <?php else: ?> 是<?php endif; ?></td> 
                <td><?php if(($spec["gs_is_sale_spec"]) == "0"): ?>否 <?php else: ?> 是<?php endif; ?></td> 
				<td><?php echo ($spec["gs_order"]); ?></td>
				<td><?php if($spec["gs_is_system_spec"] != 1): echo ($spec["gt_name"]); else: ?><span style="color:#ff0000;">所有类型共用</span><?php endif; ?></td>
                <td><input type="checkbox" id="list_<?php echo ($spec["gs_id"]); ?>" class="is_search" gsid = "<?php echo ($spec["gs_id"]); ?>" value="<?php echo ($list["gs_is_search"]); ?>" <?php if($spec["gs_is_search"] == '1'): ?>checked="checked"<?php endif; ?>/></td>
                <td>
					<?php if($spec["gs_input_type"] == 1): ?>文本输入框
					<?php elseif($spec["gs_input_type"] == 2): ?>
					下拉选框
					<?php elseif($spec["gs_input_type"] == 3): ?>
					文本域输入：textarea<?php endif; ?>
				</td>
            </tr><?php endforeach; endif; else: echo "" ;endif; ?>
            <?php if(empty($ary_spec)): ?><tr><td colspan="99" class="left">暂时没有数据!</td></tr><?php endif; ?>
        </tbody>
        <tfoot>
            <tr>
                <td colspan="99">
...
$(document).ready(function(){
	...
	$("#goods-spec-list-type-change").change(function(){
		...
		if( selected_goods_type_id > 0){
			var request_uri = "<?php echo U('Admin/GoodsProperty/specListPage');?>/gt_id/" + $(this).val();
			window.location.href = request_uri;
		}else{
			window.location.href = "<?php echo U('Admin/GoodsProperty/specListPage');?>";
		}
		return false;
	});
});
...
$(document).ready(function(){
    ...
    $('.is_search').click(function(){
        ...
        var url = "<?php echo U('Admin/GoodsProperty/doPropertyIsSearch');?>";
        $.ajax({
            url:url,
            cache:false,
            dataType:'json',
