...
<a href="<?php echo ($bread0["url"]); ?>"><?php echo ($bread0["name"]); ?></a>
 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
    ...
    <table width="100%" class="tbList">
        <thead>
            <tr class="title">
                <th colspan="99"><a href="<?php echo U('Admin/Online/pageAddCate');?>" class="first add btnG ico_add">新增在线客服分类</a></th>
            </tr>
            <tr>
                <th width="120">操作</th>
        ...
        <tbody>
           	<?php if(is_array($cateinfo)): $i = 0; $__LIST__ = $cateinfo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$category): $mod = ($i % 2 );++$i;?><tr>
                   <td width="120">
                    <a href='<?php echo U("Admin/Online/pageEditCate","ocid=$category[oc_id]");?>'>编辑</a>|
					<a href='<?php echo U("Admin/Online/doDelCate","ocid=$category[oc_id]");?>'>删除</a>
					</td>
					<td style="text-align:left" width="300"><?php echo ($category["oc_name"]); ?></td>
					<td width="120"><?php echo ($category["oc_order"]); ?></td>
					<td><?php echo ($category["oc_desc"]); ?></td>
               </tr>
				<?php if(is_array($category['sub'])): $i = 0; $__LIST__ = $category['sub'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sub): $mod = ($i % 2 );++$i;?><tr>
                   <td width="120">
                    <a href='<?php echo U("Admin/Online/pageEditCate","ocid=$sub[oc_id]");?>'>编辑</a>|
					<a href='<?php echo U("Admin/Online/doDelCate","ocid=$sub[oc_id]");?>'>删除</a>
					</td>
					<td style="text-align:left" width="300">└<?php echo ($sub["oc_name"]); ?></td>
					<td width="120"><?php echo ($sub["oc_order"]); ?></td>
					<td><?php echo ($sub["oc_desc"]); ?></td>
