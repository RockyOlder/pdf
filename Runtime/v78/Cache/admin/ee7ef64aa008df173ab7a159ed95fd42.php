 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
	...
	<table width="100%" class="tbList">
		<thead>
			<tr class="title">
				<th colspan="99">
				 <form method="post" action="<?php echo U('Admin/Try/pageList');?>">
                   <p class="conOneP" style="float: left;">
                       <a href="<?php echo U('Admin/Try/pageAdd');?>" class="btnG ico_add">添加试用</a>
                       <a href="javascript:void(0);" onclick="return getCheckedProducts(this);" class="btnG Set">批量删除</a>
                   </p>
					<span style="float: right;">
					有效期：<input type="text" class="width_100 timer search_cond" name="try_start_time" value="<?php echo ($filter["gp_start_time"]); ?>" />
					- 
					<input type="text" class="width_100 timer search_cond" name="try_end_time" value="<?php echo ($filter["gp_end_time"]); ?>" />
					<select name="field" class="small search_cond" style="width: auto">
                             ...
                             <option value="2" <?php if(($filter["field"] == 2)): ?>selected=selected<?php endif; ?> >商品编码</option>
                    </select>
    		...
    		<tbody>
        		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cp): $mod = ($i % 2 );++$i;?><tr>
        				<td>
                            <input type="checkbox" class="checkSon" name="try_id" value="<?php echo ($cp["try_id"]); ?>" />
                        </td>
        				<td>
        					<span style="display:block;width:80px;">
        					<a href='<?php echo U("Admin/Try/pageEdit?try_id=$cp[try_id]");?>' class="edit"></a> | <a href='<?php echo U("Admin/Try/doDel?try_id=$cp[try_id]");?>' class="confirm delete"></a>
                            </span>
                        </td>
        				<td><?php echo ($cp["try_title"]); ?></td>
        				<td><span class="blue"><?php echo ($cp["g_name"]); ?></span></td>
        				<td>
        				    <?php if(($cp["try_status"]) == "1"): ?><span class="green">启用</span>
                            <?php else: ?>
                                <span class="red">停用</span><?php endif; ?>
        				</td>
        				<td><?php echo ($cp["try_start_time"]); ?> - <?php echo ($cp["try_end_time"]); ?></td>
						<td><?php echo ($cp["try_num"]); ?></td>
						<td><?php echo ($cp["try_now_num"]); ?></td>
        				<td><?php echo ($cp["try_create_time"]); ?></td>
        				<td><?php echo ($cp["try_update_time"]); ?></td>
...
$(document).ready(function(){
	$("#searchButton").click(function(){
		var redirect_url = "<?php echo U('Admin/Try/pageList');?>" + "?";
...
function deleteCombiantionGoods(try_id){
    if(confirm('确定要删除吗？删除后不可恢复')){
        $.ajax({
            url:"<?php echo U('Admin/Try/doDel');?>",
            data:{'try_id':try_id},
            dateType:'json',
            type:'POST',
