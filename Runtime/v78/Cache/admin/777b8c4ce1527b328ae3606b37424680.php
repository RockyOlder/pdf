...
<a href="<?php echo ($bread0["url"]); ?>"><?php echo ($bread0["name"]); ?></a>
 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
    ...
    <!--<form method="post" action="<?php echo U('Admin/GoodsBrand/doDelBrands');?>" id="">-->
    <table width="100%" class="tbList" data-uri='<?php echo U("Admin/Email/pageList");?>'>
        <thead>
            <tr class="title">
                <th style="text-align:right;font-size: 12px;" colspan="99">
                    <form id="searchForm" method="post" href='<?php echo U("Admin/Email/pageList");?>'>
                        Email:
                        <input type="text" value="<?php echo ($ary_request['email']); ?>" name="email" class="medium">
						验证码:
						<input type="text" value="<?php echo ($ary_request['code']); ?>" name="code" class="small">
						验证状态:
						<select name="check_status">
							<option value="-1" <?php if($ary_request['check_status'] == -1 ): ?>selected<?php endif; ?>>选择</option>
							<option value="0" <?php if($ary_request['check_status'] == 0): ?>selected<?php endif; ?>>未认证</option>
							<option value="1" <?php if($ary_request['check_status'] == 1): ?>selected<?php endif; ?>>已认证</option>
							<option value="2" <?php if($ary_request['check_status'] == 2): ?>selected<?php endif; ?>>无效</option>
						</select>
						发送类型:
						<select name="email_type">
							<option value="-1" <?php if($ary_request['email_type'] == -1 ): ?>selected<?php endif; ?>>选择</option>
							<option value="0" <?php if($ary_request['email_type'] == 0): ?>selected<?php endif; ?>>普通邮件</option>
							<option value="1" <?php if($ary_request['email_type'] == 1): ?>selected<?php endif; ?>>忘记密码</option>
							<option value="2" <?php if($ary_request['email_type'] == 2): ?>selected<?php endif; ?>>邮箱验证</option>
							<option value="3" <?php if($ary_request['email_type'] == 3): ?>selected<?php endif; ?>>重置密码</option>
						</select>
						发送时间:
						<input type="text" name="start_time" class="medium timer" value="<?php echo ($ary_request["start_time"]); ?>"  >
						-
						<input type="text" name="end_time" class="medium timer" value="<?php echo ($ary_request["end_time"]); ?>"  >
        ...
        <tbody>
            <?php if(is_array($ary_email)): $i = 0; $__LIST__ = $ary_email;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$email): $mod = ($i % 2 );++$i;?><tr>
                <td><?php echo ($email['email']); ?></td>
                <td>
				<?php echo ($email['content']); ?>
				</td>
                <td>
				<?php if($email["check_status"] == 0): ?>未验证<?php endif; ?>
				<?php if($email["check_status"] == 1): ?>已验证<?php endif; ?>
				<?php if($email["check_status"] == 2): ?>失效<?php endif; ?>
				</td>
                <td><?php echo ($email["code"]); ?></td>
                <td><?php echo ($email["create_time"]); ?></td>
                <td>
					<?php echo ($email["update_time"]); ?>
                </td>
                <td>
				<?php if($email["email_type"] == 0): ?>普通邮件<?php endif; ?>
				<?php if($email["email_type"] == 1): ?>忘记密码<?php endif; ?>
				<?php if($email["email_type"] == 2): ?>邮箱验证<?php endif; ?>
				<?php if($email["email_type"] == 3): ?>重置密码<?php endif; ?>				
                </td>
            </tr><?php endforeach; endif; else: echo "" ;endif; ?>
            <?php if(empty($ary_email)): ?><tr><td colspan="99" class="left">暂时没有数据!</td></tr><?php endif; ?>
        </tbody>
        <tfoot>
		<!--
            <tr>
                <td colspan="99">
                <input type="button" data-uri='<?php echo U("Admin/GoodsBrand/doDelBrands");?>'  value="删除选中" class="btnA confirm" id="delAll" /><span class="right page"><?php echo ($page); ?></span></td>
            </tr>
		-->
        </tfoot>
    </table>
