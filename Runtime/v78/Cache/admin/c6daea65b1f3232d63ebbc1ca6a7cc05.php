...
<!--                    <p class="conOneP" style="float: left;">
                ...
                <span style="text-align:right;font-size: 12px;float: left;">
                    <form id="searchForm" method="get" action="<?php echo U('Admin/Orders/pageList');?>" style='width:55%;'>
                        ...
                        试用订单：<input type="checkbox" name="o_try" value="1" <?php if($filter['o_try'] == 1 ): ?>checked="checked"<?php endif; ?>>&nbsp;
                        第三方订单号：<input type="text" name="o_source_id" class="large" value="<?php echo ($filter["o_source_id"]); ?>" style="width: 145px;">
                        商品名称：<input type="text" name="oi_g_name" class="large" value="<?php echo ($filter["oi_g_name"]); ?>" style="width: 145px;">
                        收货人：<input type="text" name="o_receiver_name" class="large" value="<?php echo ($filter["o_receiver_name"]); ?>" style="width: 145px;">
                        收货人手机：<input type="text" name="o_receiver_mobile" class="large" value="<?php echo ($filter["o_receiver_mobile"]); ?>" style="width: 145px;">
        ...
        <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$order): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td id="oid_<?php echo ($order["o_id"]); ?>">
                    <a href="<?php echo U('Admin/Orders/pageDetails');?>?o_id=<?php echo ($order["o_id"]); ?>" style="width:120px;display:block;<?php if($order['oi_range'] == 1): ?>color:red;<?php endif; ?>">
                        <?php echo ($order["o_id"]); ?>
                    </a>
                </td>
                
                <td id="oid_<?php echo ($order["o_source_id"]); ?>" >
                    <a href="javascript:void(0);" style="width:120px;display:block;"><?php if($order["o_source_id"] != 0 ): echo ($order["o_source_id"]); endif; ?></a>
                </td>
                <td>
                <span style="width:220px;display:block;">
                    ...
                    <?php else: ?>
                         ...
                         <?php if($order["o_audit"] == '1'): ?><span style="margin-left:10px;">已审核</span>
                         <?php else: ?>
                         <?php if((($order["o_pay_status"] == '1') or ($order["o_payment"] == '6')) and($order["deliver_status"] != '已发货') and ($order["refund_status"] == '') and ($order["refund_goods_status"] == '')): ?><span style="margin-left:10px;color:blue;cursor:pointer;" class="check_audit" id="hide_audit_<?php echo ($order["o_id"]); ?>" o_id="<?php echo ($order["o_id"]); ?>" title="审核通过">未审核</span>
                         <span style="margin-left:10px;display:none;" id="check_audit_<?php echo ($order["o_id"]); ?>" >已审核</span>
                         <?php else: ?>
                            <span style="margin-left:10px;">未审核</span><?php endif; endif; ?>
                        <!-- 订单的付款状态 start -->    
                        ...
                        <?php if($order["deliver_status"] == '已发货'): ?><span style="margin-left:10px;">已发货</span>
                            <?php if($order["o_status"] == '5'): ?>已确认收货
                            <?php else: ?>
                            未确认收货<?php endif; ?>
                        <?php else: ?>
                            <span style="margin-left:-2px;"  id="unship_<?php echo ($order["o_id"]); ?>">未发货</span><?php endif; endif; ?>
                   </span> 
                </td>
                ...
                <td><?php echo ($order["o_create_time"]); ?></td>
