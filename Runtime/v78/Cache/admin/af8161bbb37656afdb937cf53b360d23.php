...
<a href="<?php echo ($bread0["url"]); ?>"><?php echo ($bread0["name"]); ?></a>
 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
    ...
    <form method="POST" action="<?php echo U('Admin/GoodsType/doDelType');?>" id="">
    <table width="100%" class="tbList">
        <thead>
            <tr class="title">
                <th colspan="99">
                    <div class="TtopLeft">
                        <a  class="btnG ico_add" href='<?php echo U("Admin/GoodsType/addGoodsType");?>'>添加商品类型</a>
                    </div>
                </th>
            </tr>
            <tr>
                ...
                <th style="text-align:left;padding-left:100px;">商品类型名称</th>
            </tr>
        </thead>
        <tbody>
            <?php if(is_array($ary_type)): $i = 0; $__LIST__ = $ary_type;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$spec): $mod = ($i % 2 );++$i;?><tr>
                <td width="20px;">
					<input type="checkbox" class="checkSon" name="gt_id[]" value="<?php echo ($spec["gt_id"]); ?>" />
				</td>
                <td width="180px;">
                    <a href='<?php echo U("Admin/GoodsType/eidtGoodsType?gtid=$spec[gt_id]");?>' class="edit" title="修改此类型"></a> 
                    <a href='<?php echo U("Admin/GoodsType/doDelTypeDetail?gtid=$spec[gt_id]");?>' class="confirm delete" title="删除此类型"></a>
					<a href='<?php echo U("Admin/GoodsProperty/addSpecPage?gt_id=$spec[gt_id]&gt_type=$spec[gt_type]");?>'>添加属性</a>
                    <a href='<?php echo U("Admin/GoodsProperty/specListPage?gt_id=$spec[gt_id]");?>'>属性列表</a>
                </td>
                <td style="text-align:left;padding-left:50px;"><?php echo ($spec["gt_name"]); ?></td>
             </tr><?php endforeach; endif; else: echo "" ;endif; ?>
            <?php if(empty($ary_type)): ?><tr><td colspan="99" class="left">暂时没有数据!</td></tr><?php endif; ?>
        </tbody>
        <tfoot>
            <tr>
                <td colspan="99">
