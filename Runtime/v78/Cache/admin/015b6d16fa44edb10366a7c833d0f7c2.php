...
<a href="<?php echo ($bread0["url"]); ?>"><?php echo ($bread0["name"]); ?></a>
 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
    ...
    <form method="get" action="<?php echo U('Admin/SalesStatistics/MembersRanking');?>" >
    <table width="100%" class="tbList">
        <thead>
            <tr class="title">
                <th colspan="99">
                   
                    <ul class="conOneUl" style="width:80%">
                        ...
                        <li>会员等级：
                            <select name="field" class="small" style="width: auto">
								...
								<?php if(is_array($level_list)): $i = 0; $__LIST__ = $level_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$lists): $mod = ($i % 2 );++$i;?><option value="<?php echo ($lists["ml_id"]); ?>" <?php if(($filter["field"]) == $lists["ml_id"]): ?>selected="selected"<?php endif; ?>><?php echo ($lists["ml_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                            </select>
                        </li>
                        <li>用户名：<input type="text" name="val" class="large" value="<?php echo ($filter["val"]); ?>" style="width: 145px;"></li>
        ...
        <tbody>
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <td>
						<?php if(($val['m_real_name'] == '' )): echo ($val["m_name"]); ?>
						<?php else: ?>
							<?php echo ($val["m_real_name"]); endif; ?> 
					</td>
					<td><?php echo (($val["count_orders"])?($val["count_orders"]):0); ?></td>
					<td><?php echo (sprintf('%.2f',$val["goods_all_price"])); ?></td>
					<td><?php echo (sprintf('%.2f',$val["o_cost_freight"])); ?></td>
					<td><?php echo (sprintf('%.2f',$val["total_amount"])); ?></td>
