...
<a href="<?php echo ($bread0["url"]); ?>"><?php echo ($bread0["name"]); ?></a>
 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
    ...
    <form id="spec_form" name="spec_form" method="post" action="<?php echo U('Admin/RefundsProperty/doEditSpec');?>">
        <table class="tbForm" width="100%">
            <thead>
                <tr class="title">
                    <th colspan="99">属性新增</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    ...
                    <td>
						<input type="text" name="gs_name" id="gs_name" class="medium" value="<?php echo ($spec["gs_name"]); ?>" /> <br> 
                    </td>
                     <td class="last">请输入属性名称，同一属性类型下，不能出现相同名称的属性。</td>
                </tr>
				<tr>
                    ...
                    <td>
						...
						<span style="margin-right:15px;">
							<input type="radio" name="gs_show_type"  value="1"  style="vertical-align:middle;" />&nbsp;退款
                            <input type="radio" name="gs_show_type"  value="2"  style="vertical-align:middle;" />&nbsp;退货
						</span>
					</td>                    
                     <td class="last"></td>
                </tr>
				<tr>
                    ...
                    <td>
						<input type="text" name="gs_order" class="medium" value="<?php echo ($spec["gs_order"]); ?>" />
					</td>                    
                ...
                <tr>
                    ...
                    <td>
                        <input type="radio" name="gs_input_type" value="1" <?php if($spec["gs_input_type"] == 1): ?>checked="checked"<?php endif; ?>  id="gs_input_type_1" />
						<label for="gs_input_type_1" title="单行文本框">文本框</label>
						<input type="radio" name="gs_input_type" value="2" <?php if($spec["gs_input_type"] == 2): ?>checked="checked"<?php endif; ?> id="gs_input_type_2" />
						<label for="gs_input_type_2" title="附件">附件</label>
						<input type="radio" name="gs_input_type" value="3" <?php if($spec["gs_input_type"] == 3): ?>checked="checked"<?php endif; ?>  id="gs_input_type_3" />
						<label for="gs_input_type_3" title="适合以一段文字的形式描述商品的某个属性">文本域</label>
                    </td>
                    <td class="last"></td>
                </tr>
				<tr style="border:1px solid #ff0000;" title="此项只是演示，添加属性时不需要处理此字段。">
                    ...
                    <td id="good_spec_style">
                        <input type="text" name="" class="medium" id="input_style_1" <?php if($spec["gs_input_type"] != 1): ?>style="display:none;"<?php endif; ?> value="请填入属性值......" readonly="readonly" />
                        <textarea name="" id="input_style_3" style="<?php if($spec["gs_input_type"] != 3): ?>display:none;<?php endif; ?>width:300px;height:60px;" readonly="readonly">请填入属性值......</textarea>
                    </td>
					<td class="last" style="color:#ff0000;">*添加或者修改退换货单时，此属性值的录入方式将以左侧显示的方式呈现，演示而已不用做处理。</td>
                </tr>
            </tbody>
            <tfoot>
                <tr>
                    <td colspan="99">
						<input type="hidden" name="gs_id" value="<?php echo ($spec["gs_id"]); ?>" />
						<input type="hidden" name="dosubmit" value="1" />
                        <input type="submit" value="保 存" class="btnA" >
                        <input type="button" value="取 消" onClick="onUrl('/Admin/RefundsProperty/specListPage');" class="btnA" >
                    </td>
                </tr>
            </tfoot>
        </table>
