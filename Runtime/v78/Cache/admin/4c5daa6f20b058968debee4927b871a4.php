...
<a href="<?php echo ($bread0["url"]); ?>"><?php echo ($bread0["name"]); ?></a>
 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
    ...
    <form id="deliveryCorp_del" method="get" action="<?php echo U('Admin/Delivery/doDel');?>">
        <table width="100%" class="tbList">
            <thead>
                <tr class="title">
                    <th colspan="99">全部物流公司列表</th>
                </tr>
                <tr>
                    ...
                    <th>物流公司</th>
                    <th>配送区域</th>
                    <th><?php echo (($pay_name)?($pay_name):'货到付款'); ?>？</th>
                </tr>
            </thead>
            <tbody>
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$dc): $mod = ($i % 2 );++$i;?><tr>
                    <td><input type="checkbox" class="checkSon" name="lc_id[]" value="<?php echo ($dc["lc_id"]); ?>" /></td>
                    <td>
                        <a href='<?php echo U("Admin/Delivery/pageEdit?lc_id=$dc[lc_id]");?>'>修改</a>&nbsp;
                        <a href='<?php echo U("Admin/Delivery/doDel?lc_id=$dc[lc_id]");?>' class="confirm">删除</a>
                    </td>
                    <td><?php if(($dc["lc_is_enable"]) == "1"): ?><span class="green">启用</span><?php else: ?><span class="red">禁用</span><?php endif; ?></td>
                    <td><?php echo ($dc["lc_name"]); ?></td>
                    <td>
                        <a href='<?php echo U("Admin/Delivery/pageListArea?lc_id=$dc[lc_id]");?>'>查看详细</a>&nbsp;
                        <a href='<?php echo U("Admin/Delivery/pageAddArea?lc_id=$dc[lc_id]");?>'>添加</a>
                    </td>
                    <td><?php if(($dc["lc_cash_on_delivery"]) == "1"): ?><span class="green">是</span><?php else: ?><span class="red">否</span><?php endif; ?></td>
