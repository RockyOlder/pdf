...
<div class="rightInner">
    <form id="Pay_List" method="post" action="<?php echo U('Admin/Financial/doSequence');?>">
        <table width="100%" class="tbList">
            ...
            <tbody>
                <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$pc): $mod = ($k % 2 );++$k;?><tr id="<?php echo ($pc["pc_abbreviation"]); ?>">
                        <td><input type="checkbox" class="pc_status" value="<?php echo ($pc["pc_abbreviation"]); ?>" <?php if(($pc["pc_status"]) == "1"): ?>checked="checked"<?php endif; ?> /></td>
                        <td><?php if(($pc["hasCfg"]) == "1"): ?><a href='<?php echo U("Admin/Financial/pageConfigOnline?code=$pc[pc_abbreviation]");?>'>编辑</a><?php endif; ?></td>
                        <td><span class="blue"><?php echo ($pc["pc_abbreviation"]); ?></span></td>
                        <td><?php echo ($pc["pc_custom_name"]); ?></td>
                        <td><?php echo ($pc["pc_last_modify"]); ?></td>
						<td>
						<?php if($pc["pc_source"] == 1 ): ?>PC端
						<?php elseif($pc["pc_source"] == 3 ): ?>
							PC端/APP
						<?php elseif($pc["pc_source"] == 4 ): ?>
							wap
						<?php elseif($pc["pc_source"] == 5 ): ?>
							PC端/wap
						<?php elseif($pc["pc_source"] == 6 ): ?>
							APP/wap
						<?php else: ?>
							PC 端/APP/wap<?php endif; ?>						
						</td>
                        <td class="left"><?php echo ($pc["pc_memo"]); ?></td>
                        <td>
                            <span name="UP_<?php echo ($pc["pc_abbreviation"]); ?>" onclick="UpSequence('<?php echo ($pc["pc_abbreviation"]); ?>')" style="cursor: pointer" > ↑</span>
                            <span name="DOWN_<?php echo ($pc["pc_abbreviation"]); ?>" onclick="DownSequence('<?php echo ($pc["pc_abbreviation"]); ?>')" style="cursor: pointer" >↓</span>
                        </td>
                        <input id="Sequence_<?php echo ($pc["pc_abbreviation"]); ?>" name="Sequence_<?php echo ($pc["pc_abbreviation"]); ?>" type="hidden" value="<?php echo ($k); ?>" />
...
<script type="text/javascript">
    $(document).ready(function(){
        $('.pc_status').click(function(){
            ...
            var url = "<?php echo U('Admin/Financial/doStatusOnline');?>";
