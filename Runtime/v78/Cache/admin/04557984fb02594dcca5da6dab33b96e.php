    ...
    <table width="100%" class="tbList">
        <thead>
            ...
            <p class="conOneP" style="float: left;">
                <a href="<?php echo U('Admin/Article/pageAdd');?>" class="btnG ico_add">新增文章</a>
            </p>
            <ul class="conOneUl" style="width:300px;">
                <form method="get" action="<?php echo U('Admin/Article/pageList');?>" >
                    <li>
                        <select name="cat_id" class="small" style="width: auto">
                            <option value='0'>请选择</option>
                            <?php if(is_array($cateinfo)): $i = 0; $__LIST__ = $cateinfo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ct): $mod = ($i % 2 );++$i;?><option value='<?php echo ($ct[cat_id]); ?>' <?php if($ct['cat_id'] == $filter['cat_id']): ?>selected="selected"<?php endif; ?>><?php echo ($ct["cat_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                        </select>
                    </li>
                    <li><input type="text" name="title" class="large" value="<?php echo ($filter["title"]); ?>" style="width: 145px;"></li>
        ...
        <tbody>
           	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$article): $mod = ($i % 2 );++$i;?><tr>
                  <td width="120">
                   <a href='<?php echo U("Admin/Article/pageEdit","aid=$article[a_id]");?>'>编辑</a>|
				<a href='<?php echo U("Admin/Article/doDel","aid=$article[a_id]");?>'>删除</a>
				</td>
				<td><?php echo ($article["a_title"]); ?></td>
				<td width="120"><?php echo ($article["cat_name"]); ?></td>
				<td width="120"><?php echo ($article['a_is_display']?'是':'否'); ?></td>
				<td width="120"><?php echo ($article["a_create_time"]); ?></td>
