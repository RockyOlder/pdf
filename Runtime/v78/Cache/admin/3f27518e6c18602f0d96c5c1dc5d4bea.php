...
<a href="<?php echo ($bread0["url"]); ?>"><?php echo ($bread0["name"]); ?></a>
 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
    ...
    <form method="get" action="<?php echo U('Admin/Promotions/pageProList');?>" id="promotion_del">
    <table width="100%" class="tbList">
        <thead>
            <tr class="title">
                <th colspan="99">
                    <p class="conOneP" style="float: left;">
                        <a href='<?php echo U("Admin/Promotions/pageProAdd");?>' class="btnG ico_add">新增促销</a>
                    </p>
                    <ul class="conOneUl" style="width:688px;">
                   
                        <li>促销时间：<input type="text" name="pmn_start_time" class="large medium timer" value="<?php echo ($filter["pmn_start_time"]); ?>"  style="width: 145px;float: none;"></li>
                        <li>至<input type="text" name="pmn_end_time" class="large medium timer" value="<?php echo ($filter["pmn_end_time"]); ?>"  style="width: 145px;float: none;"></li>
                        <li>
                            <select name="field" class="small" style="width: auto">
                                <option value="pmn_name" <?php if($filter['field'] == 'pmn_name'): ?>selected="selected"<?php endif; ?>>促销名称</option>
                                <option value="pmn_order" <?php if($filter['field'] == 'pmn_order'): ?>selected="selected"<?php endif; ?>>优先级</option>
                                <option value="pmn_class" <?php if($filter['field'] == 'pmn_class'): ?>selected="selected"<?php endif; ?>>类型代码</option>
                            </select>
                        </li>
        ...
        <tbody>
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$pmn): $mod = ($i % 2 );++$i;?><tr id="del_<?php echo ($pmn["pmn_id"]); ?>">
                    <td>
                        <input type="checkbox" class="checkSon" name="pid[]" value="<?php echo ($pmn["pmn_id"]); ?>" data-xid="checkSon_x"/>
                    </td>
                    <td>
                        <a href='<?php echo U("Admin/Promotions/pageProEdit?pid=$pmn[pmn_id]");?>'>编辑</a> 
                         <a href='javascript:;' class="Del" pid="<?php echo ($pmn["pmn_id"]); ?>">删除</a>
                    </td>
					<td>
					<?php if(($pmn["pmn_enable"]) == "1"): ?><img src="__PUBLIC__/Admin/images/span-true.png" pmn_enable="1" onclick="ajaxSetNew(<?php echo ($pmn["pmn_id"]); ?>,this);" />
					<?php else: ?>
						<img src="__PUBLIC__/Admin/images/span-false.png" pmn_enable="0" onclick="ajaxSetNew(<?php echo ($pmn["pmn_id"]); ?>,this);" /><?php endif; ?>
					</td>
                    <td>
                        <?php echo ($pmn["pmn_name"]); ?>
                    </td>
                    <td><?php echo ($pmn["pmn_order"]); ?></td>
                    <td>
                        <span class="blue" title="<?php echo ($types[$pmn['pmn_class']]['memo']); ?>"><?php echo ($pmn["pmn_class"]); ?></span>
                    </td>
                    <td><?php echo ($pmn["pmn_start_time"]); ?> - <?php echo ($pmn["pmn_end_time"]); ?></td>
                    <td class="left"><?php echo ($pmn["pmn_memo"]); ?></td>
        ...
        <tfoot>
            <tr>
                <td colspan="99"><input type="button" value="删除选中" class="btnA"  id="delAll" /><span class="right page"><?php echo ($page); ?></span></td>
            </tr>
        </tfoot>
    </table>
    </form>
...
<script type="text/javascript">
    ...
    $("input[type='button']").click(function(){
        ...
        if(confirm("确定要删除？")){
            $.ajax({
                url:"<?php echo U('Admin/Promotions/doProDel');?>",
                data:{pid:pidobj},
                type:"post",
                dataType:"json",
	...
	function ajaxSetNew(int_pmn_id,clickObj){
        ...
        if(confirm("确定要" + msg + '吗？')){
            $.post(
            '<?php echo U("Admin/Promotions/ajaxDoProEdit");?>',
