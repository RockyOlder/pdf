...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready(function(){
        ...
        $('.imageItems').click(function(){
            var http = "<?php echo U('/', '', true, false, true);?>";
...
<div class="rightInner">
    <form action="<?php echo U('Admin/Images/doDel');?>" id="imgDel" name="imgDel" method="post">
    <table width="100%" class="tbList">
        <thead>
            <tr class="title">
                <th>
                    <a class="btnG ico_refresh" href='<?php echo U("Admin/Images/doUp");?>/dir/<?php echo ($_GET['dir']); ?>'>返回上级目录</a>
                    <a class="btnG ico_refresh" href='<?php echo U("Admin/Images/pageList");?>'>返回根目录</a>
        ...
        <tbody class="nohover">
            <tr>
                <td colspan="2">
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$dir): $mod = ($i % 2 );++$i;?><div class="picList" style="padding: 0px 10px 10px 10px;height: auto;">
                        
                        <div>
                        	<?php if($dir["path_size"] != 1 and $dir["path_size"] != 0 and $dir["del"] == true): ?><div style="position: relative;float: left;top: 17px;"><input type="checkbox" name="imagesDel[]" value="<?php echo ($dir[path]); ?>" class="checkSon" /></div><?php endif; ?>           
                            <?php if(($dir["prop"]) == "dir"): ?><a href='<?php echo U("Admin/Images/pageList","dir=$dir[path]");?>'><img src="__PUBLIC__/Admin/images/dir.png" /></a>
                            <?php else: ?>
                            
                            <?php if(!empty($dir["del"])): ?><div style="position: absolute;"><input type="checkbox" class="checkSon" value="<?php echo ($dir[path]); ?>" name="imagesDel[]" /></div><?php endif; ?>
                            <img src="<?php echo (str_replace('@@','/',$dir[path])); ?>" water='<?php echo (showimage(str_replace("@@","/",$dir[path]))); ?>' style="width:120px; height: 120px; cursor: pointer;" class="imageItems" /><?php endif; ?>
                        </div>
                        <div>
                            <?php if(($dir["prop"]) == "dir"): ?><a href='<?php echo U("Admin/Images/pageList","dir=$dir[path]");?>'><?php echo ($dir["name"]); ?></a>
                            <?php else: ?>
                            <?php echo ($dir["name"]); endif; ?>
                        </div>
