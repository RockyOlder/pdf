...
<a href="<?php echo ($bread0["url"]); ?>"><?php echo ($bread0["name"]); ?></a>
 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
	...
	<table width="100%" class="tbList">
				...
				<th colspan="99" >
                    <ul class="conOneUl">
                        <form action="/Admin/Coupon/pageRulesList" id="card_search" method="get">
                            <li>名称：<input type="text" class="width_100 search_cond" name="rd_name" value="<?php echo ($filter["rd_name"]); ?>" /></li>
		...
		<form id="coupon_del" method="get" action="<?php echo U('Admin/Coupon/doRulesDel');?>">
    		<tbody>
        		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cp): $mod = ($i % 2 );++$i;?><tr>
        				<td>
                            <input type="checkbox" class="checkSon" name="rd_id[]" value="<?php echo ($cp["rd_id"]); ?>" />
                        </td>
        				<td>
        					<a href='<?php echo U("Admin/Coupon/pageRuleEdit?rd_id=$cp[rd_id]");?>' class="edit"></a> | 
                            <a href='<?php echo U("Admin/Coupon/doRulesDel?rd_id=$cp[rd_id]");?>' class="confirm delete"></a>
                        </td>
        				<td><?php echo ($cp["rd_name"]); ?></td>
        				<td><?php echo ($cp["rd_start_time"]); ?> - <?php echo ($cp["rd_end_time"]); ?></td>
        				<td><?php echo ($cp["coupon_nums"]); ?></td>
        				<td><?php echo ($cp["is_use_num"]); ?></td>
        				<td><?php echo ($cp["coupon_name"]); ?></td>
        				<td><?php echo ($cp["rd_memo"]); ?></td>
        				<td>
        					<?php if(($cp["rd_is_status"]) == "0"): ?><span class="red">未启用</span>
                            <?php else: ?>
...
$(document).ready(function(){
	$("#searchButton").click(function(){
		var redirect_url = "<?php echo U('Admin/Coupon/pageRulesList');?>" + "?";
