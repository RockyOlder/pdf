...
<a href="<?php echo ($bread0["url"]); ?>"><?php echo ($bread0["name"]); ?></a>
 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
    ...
    <table width="100%" class="tbList">
        <thead>
            <tr class="title">
                <th><a href="<?php echo U('Admin/Notice/pageAdd');?>" class="btnG ico_add">新增公告</a></th>
                <th colspan="99" style="text-align:right;font-size: 12px;">
					<form id="searchForm" method="post">
						公告标题：<input type="text" name="title" value="<?php echo ($title); ?>" class="inputClass">
        ...
        <tbody>
           	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$notice): $mod = ($i % 2 );++$i;?><tr>
                   <td width="120">
                    <a href='<?php echo U("Admin/Notice/pageEdit","pnid=$notice[pn_id]");?>'>编辑</a>|
					<a href='<?php echo U("Admin/Notice/doDel","pnid=$notice[pn_id]");?>'>删除</a>
					</td>
					<td><?php echo ($notice["pn_title"]); ?></td>
					<td width="120"><?php echo ($notice['pn_is_top']?'是':'否'); ?></td>
					<td width="200"><?php echo ($notice["pn_create_time"]); ?></td>
