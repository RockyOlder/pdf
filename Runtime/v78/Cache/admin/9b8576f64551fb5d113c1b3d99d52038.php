...
<form id="linkForm" method="post" action="<?php echo ($link?U('Admin/Source/doEdit'):U('Admin/Source/doAdd')); ?>" enctype="multipart/form-data">
<table class="tbForm" width="100%">
<thead>
    <tr class="title">
        <th colspan="99"><?php echo ($link?'修改来源':'添加来源'); ?></th>
    </tr>
</thead>
<tbody>
	<tr>
		...
		<td>
		    <input class="large" type="text" name="s_name" value="<?php echo ($link["s_name"]); ?>" id="ul_name" validate="{required:true}">
		</td>
	</tr>
    	<tr>
	    ...
	    <td>	    
			<a href="javascript:upImage();" class="btnG ico_upload">上传图片</a>
			   <span id="logo_pic">
					<img  id="show_pic" src="<?php echo (C("DOMAIN_HOST")); echo ($link["s_image_path"]); ?>" width="50" height="50"/>
					<?php if(!empty($link["s_image_path"])): ?><!--					<a id="delPic" url="<?php echo U('Admin/Links/doDel');?>?ulid=<?php echo ($link["ul_id"]); ?>" >删除</a>--><?php endif; ?>
			   </span>
			&nbsp;			
	    	<input type="hidden" id="s_image_path" name="s_image_path" value="<?php echo ($link["s_image_path"]); ?>"/>
	    </td>
	</tr>
	<tr>
	    <td class="first">排序：</td>
		<td><input class="small" type="text" name="s_order" value="<?php echo ($link["s_order"]); ?>" id="ul_order"></td>
	</tr>
</tbody>
<tfoot>
    <tr>
...
<input name="s_id" type="hidden" value="<?php echo ($link["s_id"]); ?>"/>
</form>
<div class="clear"></div>
</div>
<script type="text/javascript">
