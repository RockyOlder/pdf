 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
    ...
    <table width="100%" class="tbList">
        <thead>
            <tr class="title">
                <th colspan="99">
					 ...
					 <ul class="conOneUl" style="width:709px;">
						<form method="get" action="<?php echo U('Admin/Operation/pageList');?>">
							<li>时间：<input type="text" name="starttime" class="large medium timer" value="<?php echo ($filter["starttime"]); ?>"  style="width: 145px;float: none;"></li>
							...
							<li>
								<select name="field" class="small" style="width: auto">
									<option value="author" <?php if($filter['field'] == 'author'): ?>selected="selected"<?php endif; ?>>操作人</option>
									<option value="action" <?php if($filter['field'] == 'action'): ?>selected="selected"<?php endif; ?>>动作</option>
									<option value="content" <?php if($filter['field'] == 'content'): ?>selected="selected"<?php endif; ?>>内容</option>
								</select>
							</li>
        ...
        <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                <td><input type="checkbox" class="checkSon" name="id" value="<?php echo ($list[id]); ?>" /></td>          
