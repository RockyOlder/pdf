...
<div class="rightInner">
    <form id="setAfterSaleForm" method="post" action="<?php echo U('Admin/Orders/doAddAftersale');?>">
        <table class="tbForm" width="100%">
            <thead>
                <tr class="title">
                    <th colspan="99"><?php if(($data["value"] == '')): ?>添加售后服务配置<?php else: ?>编辑售后服务配置<?php endif; ?></th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td class="first"><i class="required ">*</i> 状态：</td>
                    <td>
            <input type="radio" name="is_set" value="1" <?php if(($data["value"] == '') OR ($data["value"] == '1')): ?>checked="checked"<?php endif; ?>>开启
                        <input type="radio" name="is_set" value="0" <?php if($data["value"] == '0'): ?>checked="checked"<?php endif; ?>>关闭
                    </td>
                    <td class="last">警告：若配置为关闭状态，那么用户将无法申请退款/货操作</td>
                </tr>
                <tr>
                    <td class="first">服务内容：</td>
                    <td colspan="2"><script id="editor" name="set_content" type="text/plain"><?php echo ($data["content"]); ?></script></td>
                </tr>
            </tbody>
            <tfoot>
                <tr>
