 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
    ...
    <table width="100%" class="tbList">
        ...
        <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>    
                ...
                <td><input type="checkbox" id="list_<?php echo ($list["id"]); ?>" bt_code="<?php echo ($list["id"]); ?>" class="status" value="<?php echo ($list["mstatus"]); ?>" <?php if($list["mstatus"] == '1'): ?>checked="checked"<?php endif; ?>/></td>
...
<script>
    $(document).ready(function(){
        ...
        $('.status').click(function(){
            ...
            var url = "<?php echo U('Admin/Menus/doStatusMenus');?>";
            $.ajax({
                url:url,
                cache:false,
                dataType:'json',
