...
<a href="<?php echo ($bread0["url"]); ?>"><?php echo ($bread0["name"]); ?></a>
 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
    ...
    <form id="type_form" name="type_form" method="post" action="<?php echo U('Admin/GoodsType/doEidtType');?>">
        <table class="tbForm" width="100%">
            <thead>
                <tr class="title">
                    <th colspan="99">商品类型编辑</th>
                </tr>
            </thead>
            <tbody>
				<tr>
                    <td class="first">商品属性类型：</td>
                    <td>
						<select name="gt_type">
							<option <?php if($ary_type["gt_type"] == 0): ?>selected<?php endif; ?> value="0">商品属性</option>
							<option <?php if($ary_type["gt_type"] == 1): ?>selected<?php endif; ?> value="1">商品试用属性<option>
						</select>
                    </td>                    
                     <td class="last">选择所属商品属性类型</td>
                </tr>
                <tr>
                    <td class="first">类型名称：</td>
                    <td>
                        <input type="text" name="gt_name" id="gt_name" class="medium" value="<?php echo ($ary_type["gt_name"]); ?>" validate="{ required:true,remote:'<?php echo U("Admin/GoodsType/checkTypeName");?>?gt_id=<?php echo ($ary_type["gt_id"]); ?>'}" /> <br> 
                    </td>                    
                     <td class="last">请输入类型名称</td>
                </tr>
            </tbody>
            <tfoot>
                <tr>
                    <td colspan="99">
					    <input type="hidden" name="gtid" value="<?php echo ($ary_type["gt_id"]); ?>">
                        <input type="submit" value="提 交" class="btnA" >
                        <input type="button" value="取 消" onClick="onUrl('<?php echo U("Admin/GoodsType/pageList");?>');" class="btnA" >
                    </td>
                </tr>
            </tfoot>
        </table>
