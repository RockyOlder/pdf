...
<a href="<?php echo ($bread0["url"]); ?>"><?php echo ($bread0["name"]); ?></a>
 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
    ...
    <table width="100%" class="tbList">
        <thead>
            <tr class="title">
                <th colspan="99"><a href="<?php echo U('Admin/Advertising/pageAdd');?>" class="btnG ico_add">新增广告</a></th>
            </tr>
            <tr>
				<th width="120">操作</th>
				<th>广告名称</th>
        ...
        <tbody>
           	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$link): $mod = ($i % 2 );++$i;?><tr>
                  <td width="120">
                   <a href='<?php echo U("Admin/Advertising/pageEdit","ulid=$link[s_id]");?>'>编辑</a>|
				<a href='<?php echo U("Admin/Advertising/doDel","s_id=$link[s_id]");?>'>删除</a>
				</td>
				<td><?php echo ($link["s_name"]); ?></td>
				<td width="120">
					<?php if($link[s_image_path]): ?><img src="<?php echo (C("DOMAIN_HOST")); echo ($link["s_image_path"]); ?>" width="120" height="50"/>				
					<?php else: endif; ?>
				</td>
			
				<td width="120"><?php echo ($link["s_order"]); ?></td>
