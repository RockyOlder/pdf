...
<div class="rightInner" id="con_tabListP_1" style=" overflow:hidden;">
    <ul class="tempUl">
        <?php if(($is_wap_lock) == "1"): ?><li>您需要开通wap模板功能，才能下载哦！</li>
            <?php else: ?>
            <?php if(!empty($data)): if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$tpl): $mod = ($i % 2 );++$i;?><li>
                        <div class="freeTem">
                            <h2><?php echo (($tpl["ti_name"])?($tpl["ti_name"]):"暂无"); ?></h2>
                            <a href='javascript:;'><img src='__FXCENTER__/<?php echo ($tpl["ti_thumbnail"]); ?>' width="225" height="260" /></a>
                        </div>
                        <p class="textCenter"><a href='javascript:void(0);' tisn="<?php echo ($tpl["tisn"]); ?>" id="a_<?php echo ($tpl["tisn"]); ?>" <?php if($tpl["local"] == '1'): ?>class="btnX"<?php else: ?>class="btnA downloadtpl"<?php endif; ?> data-uri='<?php echo U("Admin/Home/getDownloadTpl","ti_sn=$tpl[tisn]&client_sn=$tpl[cisn]&sign=$tpl[sign]&ti_category=1");?>'>下载此模板</a></p>
                    </li><?php endforeach; endif; else: echo "" ;endif; ?>
                <?php else: ?>
                <li>暂时没有wap模板可以下载！</li><?php endif; endif; ?>
    </ul>
