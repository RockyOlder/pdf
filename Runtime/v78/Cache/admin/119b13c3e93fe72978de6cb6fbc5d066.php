 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
	...
	<table width="100%" class="tbList">
		<thead>
			<tr class="title">
				<th colspan="99">
				<form method="post" action="<?php echo U('Admin/Try/apply_index');?>">
					...
					<span style="float: right;">
                        ...
                        <select name="applyStatus" class="small search_cond" style="width:auto" >
                            <option value="0" <?php if(($filter["applyStatus"] == 0)): ?>selected=selected<?php endif; ?> >未审核</option>
                            <option value="1" <?php if(($filter["applyStatus"] == 1)): ?>selected=selected<?php endif; ?> >已审核</option>
                            <option value="2" <?php if(($filter["applyStatus"] == 2)): ?>selected=selected<?php endif; ?> >拒绝</option>
                        </select>
                        <select name="field" class="small search_cond" style="width: auto">
                            <option value="1" <?php if(($filter["field"] == 1)): ?>selected=selected<?php endif; ?> >试用标题</option>
                            <option value="2" <?php if(($filter["field"] == 2)): ?>selected=selected<?php endif; ?> >商品名称</option>
                        </select>
		...
		<tbody>
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ay): $mod = ($i % 2 );++$i;?><tr>
                    <td>
                        <input type="checkbox" class="checkSon" name="tar_id" value="<?php echo ($ay["tar_id"]); ?>" />
                    </td>
                    <td>
                        <span style="display:block;width:80px;">
                            <?php if($ay['property_typeid_front'] != 0): ?><a href='javascript:void(0);' tar_id="<?php echo ($ay["tar_id"]); ?>" data-uri='<?php echo U("Admin/Try/ShowApplyQuestion");?>' class="setTry">查看问题</a>&nbsp;&nbsp;
                                <div id="children_<?php echo ($ay["tar_id"]); ?>"  style="display:none" title="回答详情"></div><?php endif; ?>
                            <?php if($ay['try_status'] == 0): ?><a href='javascript:void(0);' onclick="refuseApply('<?php echo ($ay["tar_id"]); ?>',this)">拒绝</a><?php endif; ?>
                        </span>
                    </td>
                    <td><?php echo ($ay["try_title"]); ?></td>
                    <td><?php echo ($ay["m_name"]); ?></td>
                    <td><span class="blue"><?php echo ($ay["g_name"]); ?></span></td>
                    <td>
                        <?php if($ay['try_status'] == 1): ?><span class="green">已审核</span>
                        <?php elseif($ay['try_status'] == 2): ?>
                            <span class="red">拒绝</span>
                        <?php else: ?>
                            <span class="blue">未审核</span><?php endif; ?>
                    </td>
                    <td><?php echo ($ay["tar_create_time"]); ?></td>
					<td>
                        <?php if(!empty($ay["try_oid"])): ?><a href="<?php echo U('Admin/Orders/pageDetails');?>?o_id=<?php echo ($ay["try_oid"]); ?>"><?php echo (($ay["try_oid"])?($ay["try_oid"]):'-'); ?></a><?php endif; ?>
                    </td>
...
$(document).ready(function(){
	$("#searchButton").click(function(){
		var redirect_url = "<?php echo U('Admin/Try/apply_index');?>" + "?";
...
function setAutoApply(){
    ...
    $.ajax({
        url:'<?php echo U("Admin/Try/checkAudit");?>',
        cache:false,
        dataType:"json",
...
function refuseApply(tar_id,obj){
    $.ajax({
        url:"<?php echo U('Admin/Try/doRefuseStatus');?>",
        data:{'tar_id':tar_id},
        dateType:'json',
        type:'POST',
