...
<a href="<?php echo ($bread0["url"]); ?>"><?php echo ($bread0["name"]); ?></a>
 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
    ...
    <form id="sms_set" name="sms_set" method="post" action="<?php echo U('Admin/Sms/doSetSms');?>">
        <table class="tbForm" width="100%">
            <thead>
                <tr class="title">
                    <th colspan="99">SMS设置</th>
                </tr>
            </thead>

            <tbody>
                <tr>
                    <th colspan="99">SMS基本信息</th>
                </tr>
                <tr>
                    <td class="first">接入帐号</td>
                    <td>
                        <input type="text" name="GY_SMS_NAME" id="GY_SMS_NAME"  class="medium" value="<?php echo ($GY_SMS_NAME); ?>" validate="{ required:true}" />
                    </td>
                    <td class="last"></td>
                </tr>
                <tr>
                    ...
                    <td>
                        <input type="password" name="GY_SMS_PASS" id="GY_SMS_PASS"  class="medium" value="<?php echo ($GY_SMS_PASS); ?>" validate="{ required:true}" />
                    </td>
                    <td class="last"></td>
                </tr>
                <tr>
                    <td class="first">短信余额</td>
                    <td>
                        <?php echo ($total); ?>
                    </td>
                    <td class="last"></td>
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" name="testSms" id="testSms" class="medium" validate="{ required:true}" />
                        <a href="javascript:void(0);" id="sendTestSms">发送测试短信</a>
                    </td>
                    <td class="last"></td>
                </tr>
            </tbody>
...
<script>
    $(document).ready(function(){
        $('#sendTestSms').click(function(){  
        	var GY_SMS_PASS = $('#GY_SMS_PASS').val().trim();
        	if(GY_SMS_PASS.length<=0){
        		showAlert(false,'密码不能为空');
        	}     
        	var GY_SMS_NAME = $('#GY_SMS_NAME').val().trim();
        	if(GY_SMS_NAME.length<=0){
        		showAlert(false,'接入帐号不能为空');
        	}          	       	
            var url = "<?php echo U('Admin/Sms/doTestSms');?>";
            var data = $('#sms_set').serialize();
            ajaxReturn(url,data);
        });
    });
</script>
