        ...
        <thead>
            <tr class="title">
                ...
                <th colspan="99" style="text-align:right;font-size: 12px;">
                    <form id="searchForm" method="get" action="<?php echo U('Admin/Orders/pageToAuditOrderList');?>">
        ...
        <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$order): $mod = ($i % 2 );++$i;?><tr id="o_id_<?php echo ($order["o_id"]); ?>">
                ...
                <td>
                    ...
                    <?php else: ?>        	
                        ...
                        <?php if($order["deliver_status"] != '已发货'): ?><span style="margin-left:10px;"><?php echo ($order["deliver_status"]); ?></span>
                            <?php if($order['o_audit'] == 0){ ?>
                                <span style="margin-left:10px;color:blue;cursor:pointer;" class="check_audit" id="hide_audit_<?php echo ($order["o_id"]); ?>" o_id="<?php echo ($order["o_id"]); ?>" title="审核通过">未审核</span>
                            ...
                            <?php } ?>
                        <?php else: ?>
                            <span style="margin-left:10px;"><?php echo ($order["deliver_status"]); ?></span>
                            <?php if($order['o_audit'] == 0){ ?>
