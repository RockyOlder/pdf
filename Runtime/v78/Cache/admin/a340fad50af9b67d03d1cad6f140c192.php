...
<a href="<?php echo ($bread0["url"]); ?>"><?php echo ($bread0["name"]); ?></a>
 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
    ...
    <form id="seo_edit" name="seo_edit" method="post" action="<?php echo U('Admin/Seo/doEdit');?>">
        <table class="tbForm" width="100%">
            <thead>
                <tr class="title">
                    <th colspan="99">修改SEO设置</th>
                </tr>
            </thead>

            <tbody>
                <tr>
                    <td class="first">SEO项</td>
                    <td>
                        <?php echo ($info["sc_value_desc"]); ?>
                        <input type="hidden" name="sc_key" value="<?php echo ($info["sc_key"]); ?>" />
                    </td>
                    <td class="last"></td>
                </tr>
                <tr>
                    <td class="first">SEO值</td>
                    <td>
                        <textarea class="largeBox" name="sc_value"><?php echo ($info["sc_value"]); ?></textarea>
                    </td>
                    <td class="last">SEO值在具体页面中可能会拼接一些页面数据，如商品详情页会拼接上商品名称等。多个关键词请使用英文半角逗号分隔。</td>
                </tr>
            </tbody>
