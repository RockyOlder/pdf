...
<a href="<?php echo ($bread0["url"]); ?>"><?php echo ($bread0["name"]); ?></a>
 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
    ...
    <form id="rolenode_form" name="rolenode_form" method="post" action="<?php echo U('Admin/RoleNode/doEdit');?>">
        <table class="tbForm" width="100%">
            <thead>
                <tr class="title">
                    <th colspan="99">编辑节点</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    ...
                    <td colspan="2">
                        1. 只填写模块，为模块的公共授权<br>
                        2. 填写操作与模块，为指定模块中节点的授权
                    </td>
                </tr>
                <tr>
                    <td class="first">操作：</td>
                    <td>
                        <input type="text" name="action" id="action" class="medium" value="<?php echo ($vo["action"]); ?>"/> <br> 
                    </td>
                    <td class="last">(请填写控制器的相应Action，如index,insert... 非系统管理员请勿随意修改)</td>
                </tr>
                <tr>
                    <td class="first">操作名称：</td>
                    <td>
                        <input type="text" name="action_name" id="action_name" class="medium" value="<?php echo ($vo["action_name"]); ?>"/> <br> 
                    </td>
                </tr>
                <tr>
                    <td class="first">模块：</td>
                    <td>
                        <input type="text" name="module" id="module" class="medium" value="<?php echo ($vo["module"]); ?>" validate="{ required:true,maxlength:60}"/> <br> 
                    </td>
                    <td class="last">(请填写控制器的相应名称，如RoleNode,Article... 非系统管理员请勿随意修改)</td>
                </tr>
                <tr>
                    <td class="first">模块名称：</td>
                    <td>
                        <input type="text" name="module_name" id="module_name" class="medium" value="<?php echo ($vo["module_name"]); ?>" validate="{ required:true,maxlength:60}"/> <br> 
                    </td>
                </tr>
                <tr>
                    <td class="first">节点排序：</td>
                    <td>
                        <input type="text" name="sort" id="sort" class="medium" value="<?php echo (($vo["sort"])?($vo["sort"]):'0'); ?>" validate="{ required:true,rangelength:[1,6],digits:true}"/> <br> 
                    </td>
                </tr>
                <tr>
                    <td class="first">状态：</td>
                    <td>
                        <input type="radio" value="1" <?php if($vo["status"] == '1'): ?>checked="checked"<?php endif; ?>  name="status" /> 启用 
                        <input type="radio" value="0" <?php if($vo["status"] == '0'): ?>checked="checked"<?php endif; ?> name="status" /> 禁用 
                    </td>
                </tr> 
            </tbody>
            <tfoot>
                <tr>
                    <td colspan="99">
                        <input type="hidden" value="<?php echo ($vo["id"]); ?>" name="id" id="id" />
