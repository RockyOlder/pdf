...
<a href="<?php echo ($bread0["url"]); ?>"><?php echo ($bread0["name"]); ?></a>
 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
	...
	<table width="100%" class="tbList">
		<thead>
			<tr class="title">
				<th colspan="99">
					...
					名称：<input type="text" class="width_100 search_cond" name="c_name" value="<?php echo ($filter["c_name"]); ?>" />
					券号：<input type="text" class="width_100 search_cond" name="c_sn" value="<?php echo ($filter["c_sn"]); ?>" />
					有效期：<input type="text" class="width_100 timer search_cond" name="starttime" value="<?php echo ($filter["starttime"]); ?>" />
					- 
					<input type="text" class="width_100 timer search_cond" name="endtime" value="<?php echo ($filter["endtime"]); ?>" />
					...
					<a href='<?php echo U("Admin/Coupon/pageGetExeclCoupon");?>' class="btnG ico_explort">Execl导出优惠券</a>
				</th>
			</tr>
			<tr>
		...
		<form id="coupon_del" method="get" action="<?php echo U('Admin/Coupon/doDel');?>">
    		<tbody>
        		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cp): $mod = ($i % 2 );++$i;?><tr>
        				<td>
                            <input type="checkbox" class="checkSon" name="c_id[]" value="<?php echo ($cp["c_id"]); ?>" />
                        </td>
        				<td>
        					<a href='<?php echo U("Admin/Coupon/pageEdit?c_id=$cp[c_id]");?>' class="edit"></a> | 
                            <a href='<?php echo U("Admin/Coupon/doDel?c_id=$cp[c_id]");?>' class="confirm delete"></a>
                        </td>
        				<td><?php echo ($cp["c_name"]); ?></td>
        				<td><span class="blue"><?php echo ($cp["c_sn"]); ?></span></td>
        				<td>
							<?php if($cp["c_type"] == '' or $cp["c_type"] == 0): ?>现金券
							<?php else: ?>
							折扣券<?php endif; ?>
        				</td>						
        				<td><?php echo ($cp["c_money"]); ?></td>
        				<td><?php echo ($cp["c_start_time"]); ?> - <?php echo ($cp["c_end_time"]); ?></td>
        				<td><?php echo ($cp["m_name"]); ?></td>
        				<td>
        					<?php if(($cp["c_is_use"]) == "0"): ?><span class="green">未使用</span>
                            <?php else: ?>
                                <span class="red">已使用</span><?php endif; ?>
        				</td>
...
$(document).ready(function(){
	$("#searchButton").click(function(){
		var redirect_url = "<?php echo U('Admin/Coupon/pageList');?>" + "?";
