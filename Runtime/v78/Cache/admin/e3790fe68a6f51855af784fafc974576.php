...
<a href="<?php echo ($bread0["url"]); ?>"><?php echo ($bread0["name"]); ?></a>
 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
    ...
    <form action="<?php echo U('Admin/RefundsProperty/returnReasonbulkDelete');?>" method="POST" onsubmit="return beforeDeleteConfirm();">
        <table width="100%" class="tbList">
            ...
            <tbody>
            <?php if(is_array($ary_reason)): $i = 0; $__LIST__ = $ary_reason;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$reason): $mod = ($i % 2 );++$i;?><tr>
                    <td>
                        <?php if($reason["rr_is_default"] == 0): ?><input type="checkbox" class="checkSon" name="rr_id[]"  value="<?php echo ($reason["rr_id"]); ?>" /><?php endif; ?>
                    </td>
                    <td>
                        <a href='<?php echo U("Admin/RefundsProperty/returnReasonEdit?rrid=$reason[rr_id]");?>'>编辑</a>
                        <?php if($reason["rr_is_default"] == 0): ?><a href='<?php echo U("Admin/RefundsProperty/returnReasonDelete?rrid=$reason[rr_id]");?>' class="confirm">删除</a><?php endif; ?>
                    </td>
                    <td><?php echo ($reason["rr_name"]); ?></td>
                    </td>
                    <td><?php echo ($reason["rr_order"]); ?></td>
                    <td>
                        <?php if($reason["rr_show_type"] == 1): ?>退款
                            <?php elseif($reason["rr_show_type"] == 2): ?>
                            退货<?php endif; ?>
                    </td>
                </tr><?php endforeach; endif; else: echo "" ;endif; ?>
            <?php if(empty($ary_reason)): ?><tr><td colspan="99" class="left">暂时没有数据!</td></tr><?php endif; ?>
            </tbody>
            <tfoot>
            <tr>
                <td colspan="99">
    ...
    $(document).ready(function(){
        ...
        $("#goods-spec-list-type-change").change(function(){
            ...
            if( selected_goods_type_id > 0){
                var request_uri = "<?php echo U('Admin/RefundsProperty/specListPage');?>/gt_id/" + $(this).val();
                window.location.href = request_uri;
            }else{
                window.location.href = "<?php echo U('Admin/RefundsProperty/specListPage');?>";
            }
            return false;
        });
    });
