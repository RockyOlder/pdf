...
<a href="<?php echo ($bread0["url"]); ?>"><?php echo ($bread0["name"]); ?></a>
 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
    ...
    <form method="get" action="<?php echo U('Admin/SalesStatistics/SalesRanking');?>" >
    <table width="100%" class="tbList">
        <thead>
            <tr class="title">
                <th colspan="99">
                   
                    <ul class="conOneUl" style="width:685px;">
                   
                        <li>时间：<input type="text" name="start_time" class="large medium timer" value="<?php echo ($filter["start_time"]); ?>"  style="width: 145px;float: none;"></li>
                        <li>至<input type="text" name="end_time" class="large medium timer" value="<?php echo ($filter["end_time"]); ?>"  style="width: 145px;float: none;"></li>
                        <li>
                            <select name="field" class="small" style="width: auto">
								<option value="" >全部</option>
                                <option value="1" <?php if($filter['field'] == '1'): ?>selected="selected"<?php endif; ?>>商品编码</option>
                                <option value="2" <?php if($filter['field'] == '2'): ?>selected="selected"<?php endif; ?>>货号</option>
                            </select>
                        </li>
        ...
        <tbody>
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr>
                    <td><?php echo ($val["g_sn"]); ?></td>
                    <td><?php echo ($val["gb_name"]); ?></td>
					<td><?php echo ($val["pdt_sn"]); ?></td>
					<td><?php echo ($val["g_name"]); ?></td>
					<td><?php echo (($val["total_nums"])?($val["total_nums"]):0); ?></td>
					<td><?php echo (sprintf('%.2f',$val["oi_price"])); ?></td>
					<td><?php echo (sprintf('%.2f',$val["total_price"])); ?></td>
                </tr><?php endforeach; endif; else: echo "" ;endif; ?>
            <?php if(!empty($list)): ?><tr>
                    <td colspan='4' style="font-size:13px;font-weight:bolder;">合计(已支付且不含退款/退货成功):</td>
                    <td style="font-weight: bolder;"><?php echo (($total["all_nums"])?($total["all_nums"]):0); ?></td>
                    <td style="font-weight: bolder;"><?php echo (sprintf('%.2f',$total["pdt_price"])); ?></td>
                    <td style="font-weight: bolder;"><?php echo (sprintf('%.2f',$total["total_price"])); ?></td>
                </tr><?php endif; ?>
