...
<a href="<?php echo ($bread0["url"]); ?>"><?php echo ($bread0["name"]); ?></a>
 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
	...
	<table width="100%" class="tbList">
		<thead>
			<tr class="title">
				<th colspan="99">
					...
					中奖会员：<input type="text" class="width_100 search_cond" name="m_name" value="<?php echo ($filter["m_name"]); ?>" />
					
					中奖时间：<input type="text" class="width_100 timer search_cond" name="ul_confirm_time_1" value="<?php echo ($filter["ul_confirm_time_1"]); ?>" />
					- 
					<input type="text" class="width_100 timer search_cond" name="ul_confirm_time_2" value="<?php echo ($filter["ul_confirm_time_2"]); ?>" />
					<select name="l_id" id="l_id">
						<option value="">活动名称</option>
						<?php if(is_array($lottery_data)): $i = 0; $__LIST__ = $lottery_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$lottery): $mod = ($i % 2 );++$i;?><option value="<?php echo ($lottery["l_id"]); ?>" <?php if($filter["l_id"] == $lottery.l_id): ?>selected<?php endif; ?>><?php echo ($lottery["l_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
					</select>
					<select name="ul_type" id="ul_type">
						<option value="">奖品类型</option>
						<option value="1" <?php if($filter["ul_type"] == '1'): ?>selected<?php endif; ?>>红包</option>
						<option value="2" <?php if($filter["ul_type"] == '2'): ?>selected<?php endif; ?>>神秘大奖</option>
					</select>
					红包金额：<input type="text" class="width_100 search_cond" name="ul_bonus_money" value="<?php echo ($filter["ul_bonus_money"]); ?>" />
		...
		<form id="coupon_del" method="get" action="<?php echo U('Admin/Lottery/doDel');?>">
    		<tbody>
        		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cp): $mod = ($i % 2 );++$i;?><tr>
        				<td>
                            <a href='<?php echo U("Admin/Members/pageEdit?mid=$cp[m_id]");?>'><?php echo ($cp["m_name"]); ?></a>
                        </td>
        				<td>
						<?php if(is_array($lottery_data)): $i = 0; $__LIST__ = $lottery_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$lot): $mod = ($i % 2 );++$i; if($cp['l_id'] == $lot['l_id']){echo $lot['l_name'];} endforeach; endif; else: echo "" ;endif; ?>						
						</td>
        				<td>
						<?php if($cp["ul_type"] == '1'): ?>红包<?php endif; ?>
						<?php if($cp["ul_type"] == '2'): ?>神秘大奖<?php endif; ?>						
						</td>
        				<td><?php echo ($cp["ul_bonus_money"]); ?></td>
        				<td><?php echo ($cp["ul_title"]); ?></td>
        				<td><?php echo ($cp["ul_create_time"]); ?></td>
						<td><?php echo ($cp["ul_update_time"]); ?></td>
						<td><?php echo ($cp["ul_confirm_time"]); ?></td>
...
$(document).ready(function(){
	$("#searchButton").click(function(){
		var redirect_url = "<?php echo U('Admin/Lottery/userList');?>" + "?";
