...
<a href="<?php echo ($bread0["url"]); ?>"><?php echo ($bread0["name"]); ?></a>
 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
            ...
            <div class="content">
                ...
                <div class="rightInner" data-uri='<?php echo U("Admin/Role/doEditStatus");?>'>
    ...
    <table width="100%" class="tbList">
        ...
        <form action="<?php echo U('Admin/Role/doDelete');?>" method="post">
        <tbody>
        <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                <td><input type="checkbox" class="checkSon" name="role_id[]" value="<?php echo ($list[id]); ?>" /></td>          
            <td>
            <?php if($list["r_id"] != '1'): ?><a href='<?php echo U("Admin/Role/pageEdit?id=$list[id]");?>'>修改</a>&nbsp;
                <a href='javascript:;' class="confirmurl" data-uri='<?php echo U("Admin/Role/doDelete","id=$list[id]");?>' data-msg="确定要删除“<?php echo ($list["name"]); ?>”吗？" data-acttype="ajax">删除</a>
                <?php else: ?>    
                此组为系统组，不可删除或修改<?php endif; ?>
            </td>
