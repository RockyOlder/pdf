...
<a href="<?php echo ($bread0["url"]); ?>"><?php echo ($bread0["name"]); ?></a>
 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
    ...
    <form id="coupon_edit" name="coupon_edit" method="post" action="<?php echo U('Admin/Coupon/doEdit');?>">
        <table class="tbForm" width="100%">
            ...
            <tbody>
                <tr>
                    ...
                    <td><span class="blue"><?php echo ($info["c_sn"]); ?></span></td>
                    <td class="last">不能修改</td>
                </tr>
                <tr>
                    <td class="first">* 优惠券名称</td>
                    <td>
                        <input type="text" name="c_name" class="medium" validate="{ required:true}" value="<?php echo ($info["c_name"]); ?>" />
                    </td>
                    <td class="last"></td>
                </tr>
				<tr>
                    ...
                    <td>
                       <input type="radio" value="0" name="c_type" <?php if($info["c_type"] == '' or $info["c_type"] == 0): ?>checked<?php endif; ?> />现金券
                       <input type="radio" value="1" name="c_type" <?php if($info["c_type"] == 1): ?>checked<?php endif; ?>/>折扣券			   
                    </td>
                    <td class="last"></td>
                </tr>
                <tr>
                    ...
                    <td>
                        <input type="text" name="c_money" class="medium" validate="{ required:true,number:true}" value="<?php echo ($info["c_money"]); ?>" />
                    </td>
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" name="c_start_time" id="c_start_time" class="medium timer" value="<?php echo ($info["c_start_time"]); ?>" validate="{ required:true}"/>
                    </td>
                    <td class="last"></td>
                </tr>
                <tr>
                    ...
                    <td>
                        <input type="text" name="c_end_time" id="c_end_time" class="medium timer" value="<?php echo ($info["c_end_time"]); ?>" validate="{ required:true}"/>
                    </td>
                    <td class="last"></td>
                </tr>
                <tr>
                    ...
                    <td>
                        <input type="checkbox" name="c_user_id" id="c_user_id" value="1" <?php if(($info["c_user_id"]) != "0"): ?>checked="checked"<?php endif; ?> />
                        <input type="text" id="c_user_name" name="m_name" class="medium" <?php if(($info["c_user_id"]) == "0"): ?>style="display:none;"<?php endif; ?> value="<?php echo ($info["m_name"]); ?>" validate="{ remote:'__APP__/Admin/Members/getCheckName'}" />
                    </td>
                ...
                <tr>
                    ...
                    <td>
                        <input type="checkbox" name="c_condition" id="c_condition" value="all" <?php if(($info["c_condition_money"]) == "0"): ?>checked="checked"<?php endif; ?> />
                        <span id="c_condition_money" <?php if(($info["c_condition_money"]) == "0"): ?>style="display: none;"<?php endif; ?> >订单满 
                            <input type="text" class="small" name="c_condition_money" value="<?php echo ($info["c_condition_money"]); ?>" validate="{ number:true}" /> 元才可以使用
                        </span>
                    </td>
                        ...
                        <table class="tblist">
                            <tbody>
                                <?php if(is_array($goodsgroup)): $i = 0; $__LIST__ = $goodsgroup;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 10 );++$i; if(($mod) == "0"): ?><tr><?php endif; ?>
                                    <td>
                                        <input type="checkbox" name="gg_name[]" id="gg_name" value="<?php echo ($data["gg_id"]); ?>" <?php if(in_array($data['gg_id'],$ggid))echo 'checked' ?>/> <?php echo ($data["gg_name"]); ?>
                                    </td>
                ...
                <tr>
                    ...
                    <td><textarea name="c_memo" class="mediumBox" validate="{ maxlength:200}"><?php echo ($info["c_memo"]); ?></textarea></td>
            ...
            <tfoot>
                <tr>
                    <td colspan="99">
                        <input type="hidden" name="c_id" value="<?php echo ($info["c_id"]); ?>" />
