...
<div class="rightInner">
    <form id="coupon_add" method="post" action="<?php echo U('Admin/Try/doAddSet');?>">
        <table class="tbForm" width="100%">
            ...
            <tbody>
				<?php for($n = 0; $n<5; $n++){ ?>
					<tr>
						<td class="first"><?php if($n+1 == 5){ echo "试用申请流程图：";}else{ echo "广告图片".($n+1)."：";} ?></td>
						<td>
							图片链接地址:<input type="text" name="GY_SHOP_TOP_AD_<?php echo ($n); ?>_URL" id="GY_SHOP_TOP_AD_<?php echo ($n); ?>_URL" class="medium" value="<?php echo ($ary_ads[$n+1]['ad_url']); ?>" validate="{ url:true}" />
							排序：<input type="text" name="sort_order_<?php echo ($n); ?>" value="<?php echo ($n+1); ?>" readonly='readonly' class="small" />
							<a href="javascript:upImage(<?php echo ($n); ?>);" class="btnG ico_upload">上传图片</a>
							<img src="<?php echo ($ary_ads[$n+1]['ad_pic_url']); ?>" id="SHOW_TOP_AD_<?php echo ($n); ?>" style="width:50px;height:50px;" />
							<a href="javascript:void(0);" image_id="<?php echo ($n); ?>" class="del images_tools_bar_del">删除</a>
							<input type="hidden" id="GY_SHOP_TOP_AD_<?php echo ($n); ?>" name="GY_SHOP_TOP_AD_<?php echo ($n); ?>" value="<?php echo ($ary_ads[$n+1]['ad_pic_url']); ?>"/>
						</td>
						<td class="last"></td>
					</tr>
				<?php } ?>
