...
<a href="<?php echo ($bread0["url"]); ?>"><?php echo ($bread0["name"]); ?></a>
 ...
 <?php if(($bread3["name"]) != ""): ?>&nbsp;>&nbsp;<?php echo ($bread3["name"]); endif; ?>
    ...
    <table width="100%" class="tbList">
        <thead>
            <tr class="title">
                <th colspan="99"><a href="<?php echo U('Admin/Links/pageAdd');?>" class="btnG ico_add">新增友情链接</a></th>
            </tr>
            <tr>
				<th width="120">操作</th>
				<th>链接名称</th>
				<th width="200">链接地址</th>
				<th width="120">链接LOGO</th>
				<th width="120">显示顺序</th>
            </tr>
        </thead>
        <tbody>
           	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$link): $mod = ($i % 2 );++$i;?><tr>
                  <td width="120">
                   <a href='<?php echo U("Admin/Links/pageEdit","ulid=$link[ul_id]");?>'>编辑</a>|
				<a href='<?php echo U("Admin/Links/doDel","ulid=$link[ul_id]");?>'>删除</a>
				</td>
				<td><?php echo ($link["ul_name"]); ?></td>
				<td width="200"><?php echo ($link["ul_link_url"]); ?></td>
				<td width="120">
					<?php if($link[ul_image_path]): ?><img src="<?php echo (C("DOMAIN_HOST")); echo ($link["ul_image_path"]); ?>" width="120" height="50"/>				
					<?php else: endif; ?>
				</td>
				<td width="120"><?php echo ($link["ul_order"]); ?></td>
