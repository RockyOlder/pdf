...
<form id="categoryForm" method="post" action="<?php echo ($category?U('Admin/Online/doEditCate'):U('Admin/Online/doAddCate')); ?>">
<table class="tbForm" width="100%">
<thead>
    <tr class="title">
        <th colspan="99"><?php echo ($category?'修改在线客服分类':'添加在线客服分类'); ?></th>
    </tr>
</thead>
<tbody>
	<tr>
		<td class="first">* 分类名称：</td>
		<td><input class="medium" type="text" name="oc_name" value="<?php echo ($category["oc_name"]); ?>" id="oc_name" validate="{required:true}"></td>
	</tr>
	<tr>
	    <td class="first">上级分类：</td>
	    <td>
	        <select class="medium" name="oc_parent_id">
	        	<option value="0">--作为一级分类--</option>
	        	<?php if(is_array($cateList)): $i = 0; $__LIST__ = $cateList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cate): $mod = ($i % 2 );++$i;?><option value="<?php echo ($cate["oc_id"]); ?>"<?php echo ($cate[oc_id]==$category[oc_parent_id]?' selected="selected"':''); ?>><?php echo ($cate["oc_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
	        </select>
	        <samp style="font-style:italic;">支持二级类目</samp>
		</td>
	</tr>
	<tr>
	    <td class="first">排序：</td>
		<td>
			<input class="small" type="text" name="oc_order" value="<?php echo ($category["oc_order"]); ?>" id="oc_order">
			<samp style="font-style:italic;">填写正整数，值越小，同分类下排序越靠前</samp>
		</td>
	</tr>
	<tr>
	    <td class="first">描述：</td>
		<td>
			<textarea class="mediumBox" name="oc_desc" id="oc_desc"><?php echo ($category["oc_desc"]); ?></textarea>
		</td>
	</tr>
</tbody>
<tfoot>
...
<input name="oc_id" type="hidden" value="<?php echo ($category["oc_id"]); ?>"/>
</form>
<div class="clear"></div>
</div>
<script type="text/javascript">
