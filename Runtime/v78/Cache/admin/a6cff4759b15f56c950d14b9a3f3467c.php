            ...
            <tr>
                ...
                <td>
                    <select class="medium" name="gs_gcid" id="gs_gcid">
                        ...
                        <?php if(is_array($search["cates"])): $i = 0; $__LIST__ = $search["cates"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cate): $mod = ($i % 2 );++$i;?><option value="<?php echo ($cate["gc_id"]); ?>" <?php if(($chose["gcid"]) == $cate[gc_id]): ?>selected="selected"<?php endif; ?> ><?php $__FOR_START_2130__=0;$__FOR_END_2130__=$cate[gc_level];for($i=$__FOR_START_2130__;$i < $__FOR_END_2130__;$i+=1){ ?>&nbsp;&nbsp;<?php } echo ($cate["gc_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                    </select>
                </td>
                 <td>品牌</td>						
                 <td>	
                     <select name="search_brand" id="search_brand" class="related_goods_form_info medium">
                         <option value="0"> -请选择- </option>
                         <?php if(is_array($array_brand)): $i = 0; $__LIST__ = $array_brand;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["gb_id"]); ?>" <?php if(($chose["search_brand"]) == $vo[gb_id]): ?>selected="selected"<?php endif; ?> ><?php echo ($vo["gb_name"]); ?></option>  <!-- <?php if($chose["search_brand"] == $vo.gb_id): ?>selected="selected"<?php endif; ?>  --><?php endforeach; endif; else: echo "" ;endif; ?>
                     </select>
                 </td>
                <td>商品编号</td>
                <td>
...
<div class="rightInner load" id="goodsSelecterList">
    <table width="100%" class="tbList">
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$goods): $mod = ($i % 2 );++$i;?><tr>
                <td><input type="checkbox" class="checkSon" name="gs_pdt_sn" value="<?php echo ($goods["g_sn"]); ?>" /></td>
...
function goodsSelecterSerch(){
    ...
    var url = "<?php echo U('Admin/Goods/getCollocationInfo');?>";
